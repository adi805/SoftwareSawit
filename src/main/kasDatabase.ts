...
export interface KasTransactionWithDetails extends KasTransaction {
  coa_kode?: string;
  coa_nama?: string;
...
// Transaction revision interface
export interface KasTransactionRevision {
  id: string;
  transaction_id: string;
  module: 'kas';
...
function ensureDataDirectory(): void {
  const dataDir = path.join(app.getPath('userData'), 'data', 'kas', String(currentYear));
  if (!fs.existsSync(dataDir)) {
    fs.mkdirSync(dataDir, { recursive: true });
    log.info(`[KAS-DB] Created data directory: ${dataDir}`);
  }
}

// Initialize database
export async function initKasDatabase(): Promise<void> {
  log.info('[KAS-DB] Initializing Kas database...');
  ...
  try {
    ...
    if (fs.existsSync(DB_PATH)) {
      ...
      log.info('[KAS-DB] Loaded existing database:', DB_PATH);
    } else {
      db = new SQL.Database();
      log.info('[KAS-DB] Created new database:', DB_PATH);
    }
    ...
    log.info('[KAS-DB] Kas database initialized successfully');
  } catch (error) {
    log.error('[KAS-DB] Failed to initialize database:', error);
    throw error;
  }
}
...
function createTables(): void {
  ...
  db.run(`
    CREATE TABLE IF NOT EXISTS transaction_revisions (
      id TEXT PRIMARY KEY,
      transaction_id TEXT NOT NULL,
      module TEXT NOT NULL DEFAULT 'kas',
      -- Original values (snapshot before revision)
  ...
  log.info('[KAS-DB] Tables created');
}
...
export function switchPeriod(year: number, month: number): { success: boolean; message: string } {
  ...
  try {
    ...
    if (fs.existsSync(DB_PATH)) {
      const buffer = fs.readFileSync(DB_PATH);
      db = new SQL.Database(buffer);
      log.info('[KAS-DB] Switched to existing database:', DB_PATH);
    } else {
      db = new SQL.Database();
      createTables();
      saveDatabase();
      log.info('[KAS-DB] Switched to new database:', DB_PATH);
    }

    return { success: true, message: `Switched to ${year}/${String(month).padStart(2, '0')}` };
  } catch (error) {
    log.error('[KAS-DB] Failed to switch period:', error);
...
function generateTransactionNumber(type: TransactionType): string {
  const prefix = type === TRANSACTION_TYPES.KAS_MASUK ? 'KAS-M' : 'KAS-K';
  const now = new Date();
...
export function getAllTransactions(): KasTransactionWithDetails[] {
  if (!db) return [];
  ...
  return result[0].values.map((row) => {
    ...
    return obj as unknown as KasTransactionWithDetails;
  });
}

export function getTransactionById(id: string): KasTransactionWithDetails | null {
  if (!db) return null;
...
export function createTransaction(input: CreateTransactionInput): { success: boolean; message: string; transaction?: KasTransaction } {
  if (!db) {
    return { success: false, message: 'Database not initialized' };
  }
  ...
  const transaction: KasTransaction = {
    id,
  ...
  log.info(`[KAS-DB] Created transaction: ${transactionNumber}`);
...
export function updateTransaction(
  ...
  log.info(`[KAS-DB] Updated transaction ID: ${id}`);
...
export function deleteTransaction(id: string): { success: boolean; message: string } {
  ...
  log.info(`[KAS-DB] Deleted transaction ID: ${id}`);

  return { success: true, message: 'Transaksi berhasil dihapus' };
}

export function clearAllKas(): { success: boolean; message: string; deletedCount: number } {
  if (!db) {
  ...
  log.info(`[KAS-DB] Cleared all Kas data. Deleted ${count} transactions.`);
...
export function approveTransaction(
  ...
  log.info(`[KAS-DB] Approved transaction ID: ${id}, new status: ${transaction.status === TRANSACTION_STATUS.PENDING_APPROVAL_1 ? TRANSACTION_STATUS.PENDING_APPROVAL_2 : TRANSACTION_STATUS.FULLY_APPROVED}`);
...
export function rejectTransaction(
  ...
  log.info(`[KAS-DB] Rejected transaction ID: ${id}`);
...
export function getApprovers(): Array<{ id: string; full_name: string }> {
  ...
  try {
    ...
    log.error('[KAS-DB] Failed to get approvers:', error);
    return [];
  }
}
...
export function getKasBalance(): { kasMasuk: number; kasKeluar: number; balance: number } {
  if (!db) {
  ...
  `);
  const kasMasuk = masukResult.length > 0 ? (masukResult[0].values[0][0] as number) : 0;
  ...
  `);
  const kasKeluar = keluarResult.length > 0 ? (keluarResult[0].values[0][0] as number) : 0;
...
export function getTransactionsWithPagination(
...
): { data: KasTransactionWithDetails[]; total: number; page: number; pageSize: number; totalPages: number } {
  if (!db) {
  ...
  const data: KasTransactionWithDetails[] = [];
  if (result.length > 0) {
    for (const row of result[0].values) {
      ...
      data.push(obj as unknown as KasTransactionWithDetails);
    }
  }
...
export function copyTransaction(id: string, createdBy: string): { success: boolean; message: string; transaction?: KasTransaction } {
  if (!db) {
    return { success: false, message: 'Database not initialized' };
  }
...
// ============ Import Functions ============
export interface KasImportResult {
  success: boolean;
  message: string;
...
export function importKasBatch(
  data: KasImportRow[],
  createdBy: string
): KasImportResult {
  if (!db) {
  ...
  log.info(`[KAS-DB] Import batch completed: ${imported.length} imported, ${errors.length} errors`);
...
export interface CreateRevisionInput {
  revision_reason: string;
  // Proposed changes (all optional, only changed fields need to be set)
 ...
 * Only the creator or admin can request revision.
 * Stores original snapshot and proposed changes.
 */
export function createRevisionRequest(
  transactionId: string,
  requestedBy: string,
  input: CreateRevisionInput
): { success: boolean; message: string; revision?: KasTransactionRevision } {
  if (!db) {
    return { success: false, message: 'Database not initialized' };
  }
  ...
  // Can only request revision for fully approved transactions
  ...
  // Insert revision with original values snapshot
  db.run(`
    INSERT INTO transaction_revisions (
      ...
      original_amount, original_description, original_coa_id, original_aspek_kerja_id, original_blok_id,
      proposed_transaction_date, proposed_amount, proposed_description,
      proposed_coa_id, proposed_aspek_kerja_id, proposed_blok_id,
  ...
  `, [
    ...
    transaction.coa_id,
    transaction.aspek_kerja_id,
    transaction.blok_id,
    input.proposed_transaction_date ?? null,
    input.proposed_amount ?? null,
    input.proposed_description ?? null,
    input.proposed_coa_id ?? null,
    input.proposed_aspek_kerja_id ?? null,
    input.proposed_blok_id ?? null,
    input.revision_reason.trim(),
    requestedBy,
    now,
  ...
  const revision: KasTransactionRevision = {
    id,
    ...
    revision_approver_1_id: null,
    revision_approver_1_name: null,
    revision_approver_1_at: null,
    revision_approver_2_id: null,
    revision_approver_2_name: null,
    revision_approver_2_at: null,
    rejected_by: null,
    rejected_by_name: null,
    rejected_at: null,
    rejection_reason: null,
  ...
  log.info(`[KAS-DB] Created revision request ID: ${id} for transaction: ${transaction.transaction_number}`);

  return { success: true, message: 'Pengajuan revisi berhasil', revision };
}

/**
 * Get pending revisions for a module or specific transaction
 */
export function getPendingRevisions(
  module: 'kas' | 'bank' | 'gudang' = 'kas',
  transactionId?: string
): KasTransactionRevision[] {
  if (!db) return [];
  ...
  return result[0].values.map((row) => {
    ...
    return obj as unknown as KasTransactionRevision;
  });
}
 ...
 * Get revision by ID
 */
export function getRevisionById(revisionId: string): KasTransactionRevision | null {
  if (!db) return null;
 ...
 */
export function getRevisionsForTransaction(transactionId: string): KasTransactionRevision[] {
  if (!db) return [];
...
/**
 * Approve a revision request. Requires dual approval (different approvers).
 * First approval -> Pending Revision Approval 2
 * Second approval -> Approved (and applies to transaction)
 */
export function approveRevision(
  ...
  // Cannot approve own revision request
  ...
  if (revision.status === REVISION_STATUS.PENDING_REVISION_APPROVAL_1) {
    ...
    db.run(`
      UPDATE transaction_revisions SET 
        status = ?,
        revision_approver_1_id = ?,
        revision_approver_1_name = ?,
        revision_approver_1_at = ?,
        updated_at = ?
      WHERE id = ?
    `, [
    ...
    // Second approval - Check different approver
    if (revision.revision_approver_1_id === input.approver_id) {
      return { success: false, message: 'Approver tahap 2 harus berbeda dari approver tahap 1' };
    }

    // Update revision to approved
    db.run(`
      UPDATE transaction_revisions SET 
        status = ?,
        revision_approver_2_id = ?,
        revision_approver_2_name = ?,
        revision_approver_2_at = ?,
        applied_at = ?,
        updated_at = ?
      WHERE id = ?
    `, [
    ...
    // Apply revision to original transaction
    applyRevisionToTransaction(revision, now);
  ...
  log.info(`[KAS-DB] Approved revision ID: ${revisionId}`);

  return { success: true, message: 'Revisi berhasil disetujui' };
}

/**
 * Apply revision changes to the original transaction
 */
function applyRevisionToTransaction(revision: KasTransactionRevision, appliedAt: string): void {
  if (!db) return;

  // Build update query based on proposed values
  const updates: string[] = [];
  const params: (string | number | null)[] = [];
  ...
  if (updates.length === 0) {
    log.info(`[KAS-DB] No changes to apply for revision: ${revision.id}`);
    return;
  }
  ...
  log.info(`[KAS-DB] Applied revision to transaction: ${revision.transaction_id}`);
}
...
export function rejectRevision(
  ...
  // Can only reject pending revisions
  if (revision.status !== REVISION_STATUS.PENDING_REVISION_APPROVAL_1 &&
  ...
  log.info(`[KAS-DB] Rejected revision ID: ${revisionId}`);

  return { success: true, message: 'Revisi berhasil ditolak' };
}

/**
 * Cancel a pending revision request (only by requester, only at Pending Revision Approval 1)
 */
export function cancelRevision(
  ...
  // Can only cancel at Pending Revision Approval 1
  if (revision.status !== REVISION_STATUS.PENDING_REVISION_APPROVAL_1) {
  ...
  log.info(`[KAS-DB] Cancelled revision ID: ${revisionId}`);

  return { success: true, message: 'Revisi berhasil dibatalkan' };
}

/**
 * Get revision status options
 */
...
export function addSyncStatusColumns(): void {
  ...
  if (!hasSyncStatus) {
    try {
      ...
      saveDatabase();
      log.info('[KAS-DB] Added sync status columns to transactions table');
    } catch (error) {
      log.warn('[KAS-DB] Could not add sync status columns:', error);
    }
  }
  ...
  try {
    ...
    log.warn('[KAS-DB] Could not create sync_status index:', error);
  }
}
...
export function updateSyncStatus(
  ...
  try {
    ...
    saveDatabase();
    log.info(`[KAS-DB] Updated sync status for ${recordId}: ${status} (attempts: ${newAttempts})`);
    ...
    log.error(`[KAS-DB] Failed to update sync status for ${recordId}:`, errorMsg);
...
export function getSyncStatus(recordId: string): TransactionSyncStatusRecord | null {
  ...
  try {
    ...
    log.error(`[KAS-DB] Failed to get sync status for ${recordId}:`, error);
    return null;
  }
}
...
export function getAllWithSyncStatus(): TransactionSyncStatusRecord[] {
  ...
  try {
    ...
    log.error('[KAS-DB] Failed to get all with sync status:', error);
    return [];
  }
}
 ...
 */
export function getBySyncStatus(status: TransactionSyncStatus): KasTransactionWithDetails[] {
  if (!db) return [];
...
export function closeDatabase(): void {
  if (db) {
    ...
    db.close();
    db = null;
    log.info('[KAS-DB] Database closed');
  }
}
