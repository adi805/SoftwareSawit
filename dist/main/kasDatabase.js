...
function createRevisionRequest(transactionId, requestedBy, input) {
  ...
  `, [
    ...
    electron_log_1.default.info(`[KAS-DB] Created revision request ID: ${id} for transaction: ${transaction.transaction_number}`);
    return { success: true, message: 'Pengajuan revisi berhasil', revision };
}
/**
 * Get pending revisions for a module or specific transaction
 */
function getPendingRevisions(module = 'kas', transactionId) {
    if (!db)
        return [];
    let query = `
...
function approveRevision(revisionId, input) {
    ...
    `, [
        ...
        // Add history entry
        db.run(`INSERT INTO approval_history (id, transaction_id, action, user_id, user_name, action_at, notes)
       VALUES (?, ?, ?, ?, ?, ?, ?)`, [(0, uuid_1.v4)(), revision.transaction_id, 'revision_approved_1', input.approver_id, input.approver_name, now, 'Approval revisi tahap 1']);
    }
    ...
    `, [
        ...
        // Add history entry
        db.run(`INSERT INTO approval_history (id, transaction_id, action, user_id, user_name, action_at, notes)
       VALUES (?, ?, ?, ?, ?, ?, ?)`, [(0, uuid_1.v4)(), revision.transaction_id, 'revision_approved_2', input.approver_id, input.approver_name, now, 'Approval revisi tahap 2 - Revisi diterapkan']);
    }
    else {
        return { success: false, message: `Revisi dengan status "${revision.status}" tidak dapat disetujui` };
    }
    saveDatabase();
    electron_log_1.default.info(`[KAS-DB] Approved revision ID: ${revisionId}`);
    return { success: true, message: 'Revisi berhasil disetujui' };
}
/**
 * Apply revision changes to the original transaction
 */
function applyRevisionToTransaction(revision, appliedAt) {
    if (!db)
        return;
    ...
    if (updates.length === 0) {
        electron_log_1.default.info(`[KAS-DB] No changes to apply for revision: ${revision.id}`);
        return;
    }
    updates.push('updated_at = ?');
    ...
    db.run(`UPDATE transactions SET ${updates.join(', ')} WHERE id = ?`, params);
    electron_log_1.default.info(`[KAS-DB] Applied revision to transaction: ${revision.transaction_id}`);
}
/**
...
function rejectRevision(revisionId, input) {
  ...
  `, [
    ...
    // Add history entry
    db.run(`INSERT INTO approval_history (id, transaction_id, action, user_id, user_name, action_at, notes)
     VALUES (?, ?, ?, ?, ?, ?, ?)`, [(0, uuid_1.v4)(), revision.transaction_id, 'revision_rejected', input.rejected_by_id, input.rejected_by_name, now, `Revisi ditolak: ${input.reason.trim()}`]);
    saveDatabase();
    electron_log_1.default.info(`[KAS-DB] Rejected revision ID: ${revisionId}`);
...
function cancelRevision(revisionId, cancelledBy) {
  ...
  `, [exports.REVISION_STATUS.CANCELLED, now, revisionId]);
    // Add history entry
    db.run(`INSERT INTO approval_history (id, transaction_id, action, user_id, user_name, action_at, notes)
     VALUES (?, ?, ?, ?, ?, ?, ?)`, [(0, uuid_1.v4)(), revision.transaction_id, 'revision_cancelled', cancelledBy, cancelledBy, now, 'Revisi dibatalkan oleh pengaju']);
    saveDatabase();
    electron_log_1.default.info(`[KAS-DB] Cancelled revision ID: ${revisionId}`);
...
function addSyncStatusColumns() {
    ...
    if (!hasSyncStatus) {
        try {
            ...
            saveDatabase();
            electron_log_1.default.info('[KAS-DB] Added sync status columns to transactions table');
        }
        catch (error) {
            electron_log_1.default.warn('[KAS-DB] Could not add sync status columns:', error);
        }
    }
    ...
    try {
        ...
        electron_log_1.default.warn('[KAS-DB] Could not create sync_status index:', error);
    }
}
/**
...
function updateSyncStatus(recordId, status, errorMessage) {
    ...
    try {
      ...
      WHERE id = ?`, [status, newAttempts, now, errorMessage || null, recordId]);
        saveDatabase();
        electron_log_1.default.info(`[KAS-DB] Updated sync status for ${recordId}: ${status} (attempts: ${newAttempts})`);
        return {
            success: true,
        ...
        electron_log_1.default.error(`[KAS-DB] Failed to update sync status for ${recordId}:`, errorMsg);
...
function getSyncStatus(recordId) {
    ...
    try {
       ...
       FROM transactions WHERE id = ?`, [recordId]);
        ...
        electron_log_1.default.error(`[KAS-DB] Failed to get sync status for ${recordId}:`, error);
        return null;
    }
}
/**
...
function getAllWithSyncStatus() {
    ...
    try {
       ...
       FROM transactions`);
        ...
        electron_log_1.default.error('[KAS-DB] Failed to get all with sync status:', error);
        return [];
    }
}
/**
