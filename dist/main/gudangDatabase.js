...
exports.initGudangDatabase = initGudangDatabase;
...
exports.clearAllGudang = clearAllGudang;
...
exports.getGudangStock = getGudangStock;
...
exports.importGudangBatch = importGudangBatch;
...
function ensureDataDirectory() {
    const dataDir = path.join(electron_1.app.getPath('userData'), 'data', 'gudang', String(currentYear));
    if (!fs.existsSync(dataDir)) {
        fs.mkdirSync(dataDir, { recursive: true });
        electron_log_1.default.info(`[GUDANG-DB] Created data directory: ${dataDir}`);
    }
}
// Initialize database
async function initGudangDatabase() {
    electron_log_1.default.info('[GUDANG-DB] Initializing Gudang database...');
    try {
        ...
        if (fs.existsSync(DB_PATH)) {
            ...
            electron_log_1.default.info('[GUDANG-DB] Loaded existing database:', DB_PATH);
        }
        else {
            db = new SQL.Database();
            electron_log_1.default.info('[GUDANG-DB] Created new database:', DB_PATH);
        }
        // Create tables
        createTables();
        ...
        // Save database
        saveDatabase();
        electron_log_1.default.info('[GUDANG-DB] Gudang database initialized successfully');
    }
    catch (error) {
        electron_log_1.default.error('[GUDANG-DB] Failed to initialize database:', error);
        throw error;
    }
}
function createTables() {
  ...
  `);
    ...
    electron_log_1.default.info('[GUDANG-DB] Tables created');
}
...
function switchPeriod(year, month) {
    ...
    try {
        ...
        if (fs.existsSync(DB_PATH)) {
            const buffer = fs.readFileSync(DB_PATH);
            db = new SQL.Database(buffer);
            electron_log_1.default.info('[GUDANG-DB] Switched to existing database:', DB_PATH);
        }
        else {
            db = new SQL.Database();
            createTables();
            saveDatabase();
            electron_log_1.default.info('[GUDANG-DB] Switched to new database:', DB_PATH);
        }
        return { success: true, message: `Switched to ${year}/${String(month).padStart(2, '0')}` };
    }
    catch (error) {
        electron_log_1.default.error('[GUDANG-DB] Failed to switch period:', error);
...
function generateTransactionNumber(type) {
    const prefix = type === exports.TRANSACTION_TYPES.GUDANG_MASUK ? 'GUD-M' : 'GUD-K';
    const now = new Date();
...
function createTransaction(input) {
    ...
    electron_log_1.default.info(`[GUDANG-DB] Created transaction: ${transactionNumber}`);
...
function updateTransaction(id, input, updatedBy) {
    ...
    saveDatabase();
    electron_log_1.default.info(`[GUDANG-DB] Updated transaction ID: ${id}`);
...
function deleteTransaction(id) {
    ...
    saveDatabase();
    electron_log_1.default.info(`[GUDANG-DB] Deleted transaction ID: ${id}`);
    return { success: true, message: 'Transaksi berhasil dihapus' };
}
function clearAllGudang() {
    if (!db) {
    ...
    saveDatabase();
    electron_log_1.default.info(`[GUDANG-DB] Cleared all Gudang data. Deleted ${count} transactions.`);
...
function approveTransaction(id, input) {
    ...
    saveDatabase();
    electron_log_1.default.info(`[GUDANG-DB] Approved transaction ID: ${id}, new status: ${transaction.status === exports.TRANSACTION_STATUS.PENDING_APPROVAL_1 ? exports.TRANSACTION_STATUS.PENDING_APPROVAL_2 : exports.TRANSACTION_STATUS.FULLY_APPROVED}`);
...
function rejectTransaction(id, input) {
    ...
    saveDatabase();
    electron_log_1.default.info(`[GUDANG-DB] Rejected transaction ID: ${id}`);
...
function getApprovers() {
    ...
    `);
        ...
        electron_log_1.default.error('[GUDANG-DB] Failed to get approvers:', error);
        return [];
    }
}
...
function getGudangStock() {
  ...
  `);
    const gudangMasuk = masukResult.length > 0 ? masukResult[0].values[0][0] : 0;
  ...
  `);
    const gudangKeluar = keluarResult.length > 0 ? keluarResult[0].values[0][0] : 0;
    return {
        gudangMasuk,
        gudangKeluar,
...
function importGudangBatch(data, createdBy) {
    if (!db) {
    ...
    electron_log_1.default.info(`[GUDANG-DB] Import batch completed: ${imported.length} imported, ${errors.length} errors`);
    return {
...
function createRevisionRequest(transactionId, requestedBy, input) {
  ...
  `, [
    ...
    electron_log_1.default.info(`[GUDANG-DB] Created revision request ID: ${id} for transaction: ${transaction.transaction_number}`);
    return { success: true, message: 'Pengajuan revisi berhasil', revision };
}
function getPendingRevisions(module = 'gudang', transactionId) {
    if (!db)
        return [];
    let query = `
...
function approveRevision(revisionId, input) {
    ...
    `, [exports.REVISION_STATUS.APPROVED, input.approver_id, input.approver_name, now, now, now, revisionId]);
        ...
        if (revision.proposed_item_name !== null) {
            updates.push('item_name = ?');
            params.push(revision.proposed_item_name);
        }
        if (revision.proposed_item_unit !== null) {
            updates.push('item_unit = ?');
            params.push(revision.proposed_item_unit);
        }
        if (updates.length > 0) {
            updates.push('updated_at = ?');
            params.push(now);
    ...
    saveDatabase();
    electron_log_1.default.info(`[GUDANG-DB] Approved revision ID: ${revisionId}`);
...
function rejectRevision(revisionId, input) {
  ...
  `, [exports.REVISION_STATUS.REJECTED, input.rejected_by_id, input.rejected_by_name, now, input.reason.trim(), now, revisionId]);
    ...
    saveDatabase();
    electron_log_1.default.info(`[GUDANG-DB] Rejected revision ID: ${revisionId}`);
...
function cancelRevision(revisionId, cancelledBy) {
  ...
  `, [exports.REVISION_STATUS.CANCELLED, now, revisionId]);
    ...
    saveDatabase();
    electron_log_1.default.info(`[GUDANG-DB] Cancelled revision ID: ${revisionId}`);
...
function addSyncStatusColumns() {
    ...
    if (!hasSyncStatus) {
        try {
            ...
            saveDatabase();
            electron_log_1.default.info('[GUDANG-DB] Added sync status columns to transactions table');
        }
        catch (error) {
            electron_log_1.default.warn('[GUDANG-DB] Could not add sync status columns:', error);
        }
    }
    ...
    try {
        ...
        electron_log_1.default.warn('[GUDANG-DB] Could not create sync_status index:', error);
    }
}
/**
...
function updateSyncStatus(recordId, status, errorMessage) {
    ...
    try {
      ...
      WHERE id = ?`, [status, newAttempts, now, errorMessage || null, recordId]);
        saveDatabase();
        electron_log_1.default.info(`[GUDANG-DB] Updated sync status for ${recordId}: ${status} (attempts: ${newAttempts})`);
        return {
            success: true,
        ...
        electron_log_1.default.error(`[GUDANG-DB] Failed to update sync status for ${recordId}:`, errorMsg);
...
function getSyncStatus(recordId) {
    ...
    try {
       ...
       FROM transactions WHERE id = ?`, [recordId]);
        ...
        electron_log_1.default.error(`[GUDANG-DB] Failed to get sync status for ${recordId}:`, error);
        return null;
    }
}
/**
...
function getAllWithSyncStatus() {
    ...
    try {
       ...
       FROM transactions`);
        ...
        electron_log_1.default.error('[GUDANG-DB] Failed to get all with sync status:', error);
        return [];
    }
}
/**
...
function closeDatabase() {
    if (db) {
        ...
        db.close();
        db = null;
        electron_log_1.default.info('[GUDANG-DB] Database closed');
    }
}
