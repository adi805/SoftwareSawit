...
// .wrangler/tmp/bundle-o1iV6d/checked-fetch.js
...
// .wrangler/tmp/bundle-o1iV6d/strip-cf-connecting-ip-header.js
...
var stripWeak = /* @__PURE__ */ __name((tag2) => tag2.replace(/^W\//, ""), "stripWeak");
...
var MIGRATIONS_TABLE = "_migrations";
var migrations = [
  {
...
async function runMigrations(db) {
  ...
  `);
  const appliedMigrations = await db.prepare(`SELECT name FROM ${MIGRATIONS_TABLE}`).all();
  ...
  for (const migration of migrations) {
    ...
    try {
      ...
      await db.prepare(`INSERT INTO ${MIGRATIONS_TABLE} (name) VALUES (?)`).bind(migration.name).run();
...
async function getMigrationStatus(db) {
  try {
    const result = await db.prepare(`SELECT name FROM ${MIGRATIONS_TABLE}`).all();
...
__name(getMigrationStatus, "getMigrationStatus");
var TABLE_TEMPLATES = {
  kas: `
...
function parsePeriodeToTableSuffix(yearMonth) {
  const normalized = yearMonth.replace("/", "_");
  return normalized;
}
__name(parsePeriodeToTableSuffix, "parsePeriodeToTableSuffix");
function getTableName(module, periode) {
  return `${module}_${parsePeriodeToTableSuffix(periode)}`;
}
__name(getTableName, "getTableName");
async function ensureTransactionTableExists(db, module, periode) {
  const tableName = getTableName(module, periode);
  const existing = await db.prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name = ?`).bind(tableName).first();
  if (existing) {
...
__name(ensureTransactionTableExists, "ensureTransactionTableExists");

// src/routes/kas.ts
var kasSchema = external_exports.object({
  id: external_exports.string().optional(),
  tanggal: external_exports.string(),
  kode_akun: external_exports.string(),
  uraian: external_exports.string(),
  debet: external_exports.number().default(0),
  kredit: external_exports.number().default(0),
  sync_status: external_exports.enum(["synced", "pending", "conflict", "error"]).default("pending"),
  modified_at: external_exports.string().optional(),
  device_id: external_exports.string().optional()
});
var createKasSchema = kasSchema.omit({ id: true, sync_status: true, modified_at: true });
var kasRoutes = new Hono2();
kasRoutes.get("/:year/:month", async (c) => {
  const db = c.env.DB;
  const year2 = c.req.param("year");
  const month = c.req.param("month").padStart(2, "0");
  const periode = `${year2}_${month}`;
  try {
    await ensureTransactionTableExists(db, "kas", periode);
    const page = parseInt(c.req.query("page") || "1");
    const limit = parseInt(c.req.query("limit") || "50");
    const offset = (page - 1) * limit;
    const result = await db.prepare(`SELECT * FROM kas_${periode} WHERE deleted = 0 ORDER BY tanggal DESC LIMIT ? OFFSET ?`).bind(limit, offset).all();
    const countResult = await db.prepare(`SELECT COUNT(*) as total FROM kas_${periode} WHERE deleted = 0`).first();
    return c.json({
      data: result.results,
      pagination: {
        page,
...
kasRoutes.get("/:year/:month/:id", async (c) => {
  const db = c.env.DB;
  const { year: year2, month, id } = c.req.param();
  const periode = `${year2}_${month.padStart(2, "0")}`;
  try {
    const result = await db.prepare(`SELECT * FROM kas_${periode} WHERE id = ? AND deleted = 0`).bind(id).first();
    if (!result) {
...
kasRoutes.post("/:year/:month", zValidator("json", createKasSchema), async (c) => {
  ...
  try {
    ...
    await db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?)
      `).bind(id, body.tanggal, body.kode_akun, body.uraian, body.debet || 0, body.kredit || 0, now, now, now).run();
    const result = await db.prepare(`SELECT * FROM kas_${periode} WHERE id = ?`).bind(id).first();
...
kasRoutes.put("/:year/:month/:id", zValidator("json", kasSchema.partial()), async (c) => {
  ...
  try {
    ...
    values.push(now);
    values.push(id);
    await db.prepare(`UPDATE kas_${periode} SET ${updates.join(", ")} WHERE id = ?`).bind(...values).run();
    const result = await db.prepare(`SELECT * FROM kas_${periode} WHERE id = ?`).bind(id).first();
    return c.json(result);
  } catch (error) {
...
kasRoutes.delete("/:year/:month/:id", async (c) => {
  ...
  try {
    const now = (/* @__PURE__ */ new Date()).toISOString();
    await db.prepare(`UPDATE kas_${periode} SET deleted = 1, sync_status = 'pending', modified_at = ? WHERE id = ?`).bind(now, id).run();
...
kasRoutes.get("/:year/:month/changes", async (c) => {
  ...
  try {
    await ensureTransactionTableExists(db, "kas", periode);
    if (since) {
      const result = await db.prepare(`SELECT * FROM kas_${periode} WHERE modified_at > ? ORDER BY modified_at ASC`).bind(since).all();
      return c.json({
        data: result.results,
        since,
...
// src/routes/bank.ts
var bankSchema = external_exports.object({
  id: external_exports.string().optional(),
  tanggal: external_exports.string(),
  kode_akun: external_exports.string(),
  uraian: external_exports.string(),
...
var createBankSchema = bankSchema.omit({ id: true, sync_status: true, modified_at: true });
var bankRoutes = new Hono2();
bankRoutes.get("/:year/:month", async (c) => {
  ...
  try {
    ...
    const limit = parseInt(c.req.query("limit") || "50");
    const offset = (page - 1) * limit;
    const result = await db.prepare(`SELECT * FROM bank_${periode} WHERE deleted = 0 ORDER BY tanggal DESC LIMIT ? OFFSET ?`).bind(limit, offset).all();
    const countResult = await db.prepare(`SELECT COUNT(*) as total FROM bank_${periode} WHERE deleted = 0`).first();
    return c.json({
      data: result.results,
      pagination: {
        page,
...
bankRoutes.get("/:year/:month/:id", async (c) => {
  ...
  const periode = `${year2}_${month.padStart(2, "0")}`;
  try {
    const result = await db.prepare(`SELECT * FROM bank_${periode} WHERE id = ? AND deleted = 0`).bind(id).first();
    if (!result) {
      return c.json({ error: "Record not found" }, 404);
    }
    return c.json(result);
...
bankRoutes.post("/:year/:month", zValidator("json", createBankSchema), async (c) => {
  ...
  try {
    ...
    await db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?)
      `).bind(id, body.tanggal, body.kode_akun, body.uraian, body.debet || 0, body.kredit || 0, now, now, now).run();
    const result = await db.prepare(`SELECT * FROM bank_${periode} WHERE id = ?`).bind(id).first();
    return c.json(result, 201);
  } catch (error) {
...
bankRoutes.put("/:year/:month/:id", zValidator("json", bankSchema.partial()), async (c) => {
  ...
  try {
    ...
    values.push(now);
    values.push(id);
    await db.prepare(`UPDATE bank_${periode} SET ${updates.join(", ")} WHERE id = ?`).bind(...values).run();
    const result = await db.prepare(`SELECT * FROM bank_${periode} WHERE id = ?`).bind(id).first();
    return c.json(result);
  } catch (error) {
...
bankRoutes.delete("/:year/:month/:id", async (c) => {
  ...
  try {
    const now = (/* @__PURE__ */ new Date()).toISOString();
    await db.prepare(`UPDATE bank_${periode} SET deleted = 1, sync_status = 'pending', modified_at = ? WHERE id = ?`).bind(now, id).run();
    return c.json({ success: true, message: "Record deleted" });
  } catch (error) {
...
bankRoutes.get("/:year/:month/changes", async (c) => {
  ...
  try {
    await ensureTransactionTableExists(db, "bank", periode);
    if (since) {
      const result = await db.prepare(`SELECT * FROM bank_${periode} WHERE modified_at > ? ORDER BY modified_at ASC`).bind(since).all();
      return c.json({
        data: result.results,
        since,
        timestamp: (/* @__PURE__ */ new Date()).toISOString()
...
// src/routes/gudang.ts
var gudangSchema = external_exports.object({
  id: external_exports.string().optional(),
  tanggal: external_exports.string(),
  kode_barang: external_exports.string(),
  nama_barang: external_exports.string(),
  quantity: external_exports.number().default(0),
  satuan: external_exports.string().default("unit"),
  harga_satuan: external_exports.number().default(0),
  total_harga: external_exports.number().default(0),
  jenis_transaksi: external_exports.enum(["masuk", "keluar"]).default("masuk"),
  uraian: external_exports.string().optional(),
  sync_status: external_exports.enum(["synced", "pending", "conflict", "error"]).default("pending"),
  modified_at: external_exports.string().optional(),
  device_id: external_exports.string().optional()
});
var createGudangSchema = gudangSchema.omit({ id: true, sync_status: true, modified_at: true });
var gudangRoutes = new Hono2();
gudangRoutes.get("/:year/:month", async (c) => {
  ...
  try {
    ...
    const limit = parseInt(c.req.query("limit") || "50");
    const offset = (page - 1) * limit;
    const result = await db.prepare(`SELECT * FROM gudang_${periode} WHERE deleted = 0 ORDER BY tanggal DESC LIMIT ? OFFSET ?`).bind(limit, offset).all();
    const countResult = await db.prepare(`SELECT COUNT(*) as total FROM gudang_${periode} WHERE deleted = 0`).first();
    return c.json({
      data: result.results,
      pagination: {
        page,
...
gudangRoutes.get("/:year/:month/:id", async (c) => {
  ...
  const periode = `${year2}_${month.padStart(2, "0")}`;
  try {
    const result = await db.prepare(`SELECT * FROM gudang_${periode} WHERE id = ? AND deleted = 0`).bind(id).first();
    if (!result) {
      return c.json({ error: "Record not found" }, 404);
    }
    return c.json(result);
...
gudangRoutes.post("/:year/:month", zValidator("json", createGudangSchema), async (c) => {
  ...
  try {
    ...
    const result = await db.prepare(`SELECT * FROM gudang_${periode} WHERE id = ?`).bind(id).first();
    return c.json(result, 201);
  } catch (error) {
...
gudangRoutes.put("/:year/:month/:id", zValidator("json", gudangSchema.partial()), async (c) => {
  ...
  try {
    ...
    if (body.kode_barang !== void 0) {
      updates.push("kode_barang = ?");
      values.push(body.kode_barang);
    }
    if (body.nama_barang !== void 0) {
    ...
    values.push(now);
    values.push(id);
    await db.prepare(`UPDATE gudang_${periode} SET ${updates.join(", ")} WHERE id = ?`).bind(...values).run();
    const result = await db.prepare(`SELECT * FROM gudang_${periode} WHERE id = ?`).bind(id).first();
    return c.json(result);
  } catch (error) {
...
gudangRoutes.delete("/:year/:month/:id", async (c) => {
  ...
  try {
    const now = (/* @__PURE__ */ new Date()).toISOString();
    await db.prepare(`UPDATE gudang_${periode} SET deleted = 1, sync_status = 'pending', modified_at = ? WHERE id = ?`).bind(now, id).run();
    return c.json({ success: true, message: "Record deleted" });
  } catch (error) {
...
gudangRoutes.get("/:year/:month/changes", async (c) => {
  ...
  try {
    await ensureTransactionTableExists(db, "gudang", periode);
    if (since) {
      const result = await db.prepare(`SELECT * FROM gudang_${periode} WHERE modified_at > ? ORDER BY modified_at ASC`).bind(since).all();
      return c.json({
        data: result.results,
        since,
        timestamp: (/* @__PURE__ */ new Date()).toISOString()
...
// src/routes/auth.ts
var JWT_SECRET = new TextEncoder().encode("softwaresawit-secret-key-change-in-production");
...
var registerSchema = external_exports.object({
  ...
  role: external_exports.enum(["admin", "user", "approver"]).default("user"),
  modules: external_exports.array(external_exports.enum(["kas", "bank", "gudang"])).default(["kas"])
});
...
var verifyToken = /* @__PURE__ */ __name(async (token) => {
  try {
...
authRoutes.post("/login", zValidator("json", loginSchema), async (c) => {
  ...
  const body = c.req.valid("json");
  try {
    const user = await db.prepare("SELECT * FROM users WHERE username = ? AND password = ? AND deleted = 0").bind(body.username, body.password).first();
    if (!user) {
    ...
    if (body.device_id) {
      await db.prepare(`
        ...
        `).bind(body.device_id, user.id).run();
    }
    return c.json({
      token,
      user: {
...
authRoutes.post("/register", zValidator("json", registerSchema), async (c) => {
  ...
  try {
    ...
    await db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `).bind(id, body.username, body.password, body.nama, body.role, JSON.stringify(body.modules), now, now).run();
...
var coaSchema = external_exports.object({
  id: external_exports.string().optional(),
  kode: external_exports.string(),
  nama: external_exports.string(),
  jenis: external_exports.enum(["asset", "liability", "equity", "revenue", "expense"]),
  kategori: external_exports.string().optional(),
  parent_kode: external_exports.string().optional(),
  sync_status: external_exports.enum(["synced", "pending", "conflict", "error"]).default("pending")
});
var createCoaSchema = coaSchema.omit({ id: true, sync_status: true });
var blokSchema = external_exports.object({
  id: external_exports.string().optional(),
  kode_blok: external_exports.string(),
  nama: external_exports.string(),
  tahun_tanam: external_exports.number(),
  luas: external_exports.number(),
  status: external_exports.enum(["TM", "TBM", "TTM", "TLS"]),
  keterangan: external_exports.string().optional(),
  pokok: external_exports.number().optional(),
  sph: external_exports.number().optional(),
  bulan_tanam: external_exports.string().optional(),
  status_tanaman: external_exports.string().optional(),
  sync_status: external_exports.enum(["synced", "pending", "conflict", "error"]).default("pending")
});
var createBlokSchema = blokSchema.omit({ id: true, sync_status: true });
var aspekKerjaSchema = external_exports.object({
  id: external_exports.string().optional(),
  kode: external_exports.string(),
  nama: external_exports.string(),
  kategori: external_exports.string().optional(),
...
var createAspekKerjaSchema = aspekKerjaSchema.omit({ id: true, sync_status: true });
var masterRoutes = new Hono2();
masterRoutes.get("/coa", async (c) => {
  const db = c.env.DB;
  try {
    const result = await db.prepare("SELECT * FROM coa WHERE deleted = 0 ORDER BY kode ASC").all();
...
masterRoutes.get("/coa/:id", async (c) => {
  ...
  try {
    const result = await db.prepare("SELECT * FROM coa WHERE id = ? AND deleted = 0").bind(id).first();
    if (!result) {
...
masterRoutes.post("/coa", zValidator("json", createCoaSchema), async (c) => {
  ...
  try {
    ...
    await db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, 'pending', ?, ?)
      `).bind(id, body.kode, body.nama, body.jenis, body.kategori || "", body.parent_kode || "", now, now).run();
    const result = await db.prepare("SELECT * FROM coa WHERE id = ?").bind(id).first();
    return c.json(result, 201);
  } catch (error) {
...
masterRoutes.put("/coa/:id", zValidator("json", coaSchema.partial()), async (c) => {
  ...
  try {
    ...
    if (body.jenis !== void 0) {
      updates.push("jenis = ?");
      values.push(body.jenis);
    }
    if (body.kategori !== void 0) {
      updates.push("kategori = ?");
      values.push(body.kategori);
    }
    if (body.parent_kode !== void 0) {
      updates.push("parent_kode = ?");
      values.push(body.parent_kode);
    }
    if (body.sync_status !== void 0) {
      updates.push("sync_status = ?");
      values.push(body.sync_status);
    ...
    values.push(now);
    values.push(id);
    await db.prepare(`UPDATE coa SET ${updates.join(", ")} WHERE id = ?`).bind(...values).run();
    const result = await db.prepare("SELECT * FROM coa WHERE id = ?").bind(id).first();
    return c.json(result);
  } catch (error) {
...
masterRoutes.delete("/coa/:id", async (c) => {
  ...
  try {
    const now = (/* @__PURE__ */ new Date()).toISOString();
    await db.prepare(`UPDATE coa SET deleted = 1, sync_status = 'pending', updated_at = ? WHERE id = ?`).bind(now, id).run();
...
masterRoutes.get("/blok", async (c) => {
  const db = c.env.DB;
  try {
    const result = await db.prepare("SELECT * FROM blok WHERE deleted = 0 ORDER BY kode_blok ASC").all();
    return c.json({ data: result.results });
  } catch (error) {
...
masterRoutes.get("/blok/:id", async (c) => {
  ...
  const { id } = c.req.param();
  try {
    const result = await db.prepare("SELECT * FROM blok WHERE id = ? AND deleted = 0").bind(id).first();
    if (!result) {
...
masterRoutes.post("/blok", zValidator("json", createBlokSchema), async (c) => {
  ...
  try {
    ...
    const result = await db.prepare("SELECT * FROM blok WHERE id = ?").bind(id).first();
    return c.json(result, 201);
  } catch (error) {
...
masterRoutes.put("/blok/:id", zValidator("json", blokSchema.partial()), async (c) => {
  ...
  try {
    ...
    const updates = [];
    const values = [];
    if (body.kode_blok !== void 0) {
      updates.push("kode_blok = ?");
      values.push(body.kode_blok);
    }
    if (body.nama !== void 0) {
      updates.push("nama = ?");
      values.push(body.nama);
    }
    if (body.tahun_tanam !== void 0) {
      updates.push("tahun_tanam = ?");
      values.push(body.tahun_tanam);
    }
    if (body.luas !== void 0) {
      updates.push("luas = ?");
      values.push(body.luas);
    }
    ...
    if (body.keterangan !== void 0) {
      ...
      values.push(body.keterangan);
    }
    if (body.pokok !== void 0) {
      updates.push("pokok = ?");
      values.push(body.pokok);
    }
    if (body.sph !== void 0) {
      updates.push("sph = ?");
      values.push(body.sph);
    }
    if (body.bulan_tanam !== void 0) {
      updates.push("bulan_tanam = ?");
      values.push(body.bulan_tanam);
    }
    if (body.status_tanaman !== void 0) {
      updates.push("status_tanaman = ?");
      values.push(body.status_tanaman);
    }
    if (body.sync_status !== void 0) {
      updates.push("sync_status = ?");
      values.push(body.sync_status);
    ...
    values.push(now);
    values.push(id);
    await db.prepare(`UPDATE blok SET ${updates.join(", ")} WHERE id = ?`).bind(...values).run();
    const result = await db.prepare("SELECT * FROM blok WHERE id = ?").bind(id).first();
    return c.json(result);
  } catch (error) {
...
masterRoutes.delete("/blok/:id", async (c) => {
  ...
  try {
    const now = (/* @__PURE__ */ new Date()).toISOString();
    await db.prepare(`UPDATE blok SET deleted = 1, sync_status = 'pending', updated_at = ? WHERE id = ?`).bind(now, id).run();
...
masterRoutes.get("/aspek-kerja", async (c) => {
  const db = c.env.DB;
  try {
    const result = await db.prepare("SELECT * FROM aspek_kerja WHERE deleted = 0 ORDER BY kode ASC").all();
    return c.json({ data: result.results });
  } catch (error) {
...
masterRoutes.get("/aspek-kerja/:id", async (c) => {
  ...
  const { id } = c.req.param();
  try {
    const result = await db.prepare("SELECT * FROM aspek_kerja WHERE id = ? AND deleted = 0").bind(id).first();
    if (!result) {
...
masterRoutes.post("/aspek-kerja", zValidator("json", createAspekKerjaSchema), async (c) => {
  ...
  try {
    ...
    await db.prepare(`
        ...
        VALUES (?, ?, ?, ?, 'pending', ?, ?)
      `).bind(id, body.kode, body.nama, body.kategori || "", now, now).run();
    const result = await db.prepare("SELECT * FROM aspek_kerja WHERE id = ?").bind(id).first();
    return c.json(result, 201);
  } catch (error) {
...
masterRoutes.put("/aspek-kerja/:id", zValidator("json", aspekKerjaSchema.partial()), async (c) => {
  ...
  try {
    ...
    values.push(now);
    values.push(id);
    await db.prepare(`UPDATE aspek_kerja SET ${updates.join(", ")} WHERE id = ?`).bind(...values).run();
    const result = await db.prepare("SELECT * FROM aspek_kerja WHERE id = ?").bind(id).first();
    return c.json(result);
  } catch (error) {
...
masterRoutes.delete("/aspek-kerja/:id", async (c) => {
  ...
  try {
    const now = (/* @__PURE__ */ new Date()).toISOString();
    await db.prepare(`UPDATE aspek_kerja SET deleted = 1, sync_status = 'pending', updated_at = ? WHERE id = ?`).bind(now, id).run();
...
// .wrangler/tmp/bundle-o1iV6d/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-o1iV6d/middleware-loader.entry.ts
