...
class CrossTests {
  ...
  async testModuleIndependence() {
    ...
    await this.runAssertion('VAL-CROSS-001: Module independence', async () => {
      ...
      for (const module of modules) {
        const menu = await this.window.locator(`a:has-text("${module}"), [data-testid="${module.toLowerCase()}-menu"]`).first();
  ...
  async testMasterDataReferences() {
    ...
    await this.runAssertion('VAL-CROSS-006: COA deletion protection', async () => {
      ...
      const coaLink = await this.window.locator('a:has-text("COA"), [data-testid="coa-menu"]').first();
    ...
    await this.runAssertion('VAL-CROSS-007: Aspek Kerja deletion protection', async () => {
      const akLink = await this.window.locator('a:has-text("Aspek Kerja"), [data-testid="aspek-kerja-menu"]').first();
  ...
  async testReporting() {
    ...
    await this.runAssertion('VAL-CROSS-013: Kas report generation', async () => {
      ...
      if (await kasMenu.count() > 0) {
        ...
        const reportBtn = await this.window.locator('button:has-text("Report"), button:has-text("Laporan"), [data-testid="generate-report"]').first();
    ...
    await this.runAssertion('VAL-CROSS-023: Period switching', async () => {
      const periodSelect = await this.window.locator('select[name="period"], select[name="periode"], [data-testid="period-select"]').first();
    ...
    await this.runAssertion('VAL-CROSS-026: Module-specific approval permissions', async () => {
      ...
      for (const module of modules) {
        ...
        if (await menu.count() > 0) {
          ...
          const approveBtn = await this.window.locator('button:has-text("Approve"), button:has-text("Setuju"), .approve-btn').first();
