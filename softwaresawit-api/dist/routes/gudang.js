...
export const gudangRoutes = new Hono();
...
gudangRoutes.post('/:year/:month', zValidator('json', createGudangSchema), async (c) => {
    ...
    try {
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?)
      `)
            .bind(id, body.tanggal, body.kode_barang, body.nama_barang, body.quantity || 0, body.satuan || 'unit', body.harga_satuan || 0, totalHarga, body.jenis_transaksi || 'masuk', body.uraian || '', now, now, now)
            .run();
        const result = await db
