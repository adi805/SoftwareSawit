...
export interface ResolutionResult {
  success: boolean;
  resolution: 'local' | 'remote' | 'merged' | 'manual_pending';
...
// Manual resolution callback type
...
export function detectConflict(
  ...
  if (timestampDiff > CONFLICT_THRESHOLD_MS) {
    ...
    if (dataDiffers) {
      ...
      const conflictType: ConflictType = 'edit_edit';
 ...
 */
export async function applyResolutionStrategy(
  module: string,
  conflict: ConflictRecord,
  strategy: ResolutionStrategy,
  manualCallback?: ManualResolutionCallback
): Promise<ResolutionResult> {
  log.info(`[ConflictResolution] Applying strategy '${strategy}' for ${module}/${conflict.recordId}`);
  ...
  switch (strategy) {
    ...
    case 'manual':
      if (!manualCallback) {
        log.error('[ConflictResolution] Manual resolution callback not provided');
        return {
          success: false,
    ...
    default:
      log.warn(`[ConflictResolution] Unknown strategy '${strategy}', falling back to last_write_wins`);
      return resolveLastWriteWins(conflict);
  }
}
 ...
 */
function resolveLastWriteWins(conflict: ConflictRecord): ResolutionResult {
 ...
 */
function resolveLocalWins(conflict: ConflictRecord): ResolutionResult {
  log.info(`[ConflictResolution] Resolving with local version: ${conflict.recordId}`);

  // Log the conflict resolution
  const conflictId = logConflictResolution(
    conflict.module,
    conflict.recordId,
 ...
 */
function resolveRemoteWins(conflict: ConflictRecord): ResolutionResult {
  log.info(`[ConflictResolution] Resolving with remote version: ${conflict.recordId}`);
 ...
 */
function resolveMerge(conflict: ConflictRecord): ResolutionResult {
  log.info(`[ConflictResolution] Performing merge for: ${conflict.recordId}`);

  const mergedData: Record<string, unknown> = {};
  const fieldConflicts = getConflictingFields(
  ...
  for (const key of allKeys) {
    ...
    if (hasConflict) {
      ...
      log.info(`[ConflictResolution] Field '${key}' conflicts, using local value`);
    } else {
...
async function resolveManually(
  module: string,
  conflict: ConflictRecord,
  callback: ManualResolutionCallback
): Promise<ResolutionResult> {
  log.info(`[ConflictResolution] Queuing for manual resolution: ${module}/${conflict.recordId}`);
  ...
  try {
    ...
    if (result.success && conflictId) {
      ...
      if (result.resolution !== 'manual_pending') {
        updateConflictRecordResolution(
          conflictId,
          result.resolution as 'local' | 'remote' | 'merged',
          result.resolvedData
        );
      }
    }
    ...
    log.error('[ConflictResolution] Manual resolution failed:', error);
    return {
      success: false,
      resolution: 'manual_pending',
...
function logConflictResolution(
  ...
  try {
    ...
    if ('logConflict' in syncDb && typeof syncDb.logConflict === 'function') {
      ...
      log.info(`[CONFLICT_LOG] ${id}|${module}|${recordId}|${conflictType}|${resolvedWith}|${localTimestamp}|${remoteTimestamp}|${now}`);
    }

    log.info(`[ConflictResolution] Logged conflict resolution: ${id}`);
    return id;
  } catch (error) {
    log.error('[ConflictResolution] Failed to log conflict resolution:', error);
    return id;
  }
}
 ...
 */
function createConflictRecord(module: string, conflict: ConflictRecord): string {
  const id = generateId();
  const now = new Date().toISOString();
  ...
  try {
    ...
    log.info(`[ConflictResolution] Created conflict record: ${id}`);
    return id;
  } catch (error) {
    log.error('[ConflictResolution] Failed to create conflict record:', error);
    return id;
  }
}
 ...
 */
function updateConflictRecordResolution(
  conflictId: string,
  ...
  try {
    ...
    log.info(`[ConflictResolution] Updated conflict record: ${conflictId}`);
  } catch (error) {
    log.error('[ConflictResolution] Failed to update conflict record:', error);
  }
}
 ...
 */
export function getPendingConflicts(module?: string): ConflictRecord[] {
  try {
    ...
    log.error('[ConflictResolution] Failed to get pending conflicts:', error);
    return [];
  }
}
 ...
 */
export function getRecentConflicts(limit: number = 50): ConflictRecord[] {
  try {
    ...
    log.error('[ConflictResolution] Failed to get recent conflicts:', error);
    return [];
  }
}
...
export function getConflictStats(): ConflictStats {
  try {
    ...
    log.error('[ConflictResolution] Failed to get conflict stats:', error);
    return {
      total: 0,
      pendingManual: 0,
      resolvedLocal: 0,
 ...
 */
function mapToConflictRecord(dbConflict: syncDb.ConflictRecordFromDb): ConflictRecord {
  return {
    id: dbConflict.id,
    module: dbConflict.module,
    recordId: dbConflict.recordId,
    conflictType: dbConflict.conflictType as ConflictType,
    localVersion: {
      recordId: dbConflict.recordId,
    ...
    resolvedWith: dbConflict.resolvedWith as 'local' | 'remote' | 'merged' | undefined,
...
export function resolveConflictManually(
  ...
  try {
    ...
    log.info(`[ConflictResolution] Manually resolved conflict: ${conflictId} -> ${resolution}`);
    return { success: true, message: 'Conflict resolved successfully' };
  } catch (error) {
    log.error('[ConflictResolution] Failed to resolve conflict manually:', error);
    return {
      success: false,
...
export function discardConflict(
  ...
  try {
    ...
    log.info(`[ConflictResolution] Discarded conflict: ${conflictId}`);
    return { success: true, message: 'Conflict discarded' };
  } catch (error) {
    log.error('[ConflictResolution] Failed to discard conflict:', error);
    return {
      success: false,
 ...
 */
export async function processQueueItemForConflict(
  module: string,
  queueItem: syncDb.SyncQueueItem,
  remoteTimestamp?: string
): Promise<{
  shouldSync: boolean;
  ...
  const localData = JSON.parse(queueItem.data) as Record<string, unknown>;
  ...
  const remoteVersion: RemoteVersion = {
    recordId: queueItem.recordId,
  ...
  log.warn(`[ConflictResolution] Conflict detected: ${module}/${queueItem.recordId}`);
 ...
 */
function getDefaultStrategy(): ResolutionStrategy {
 ...
 */
let globalStrategy: ResolutionStrategy = 'last_write_wins';
const moduleStrategies: Record<string, ResolutionStrategy> = {};

export function setResolutionStrategy(strategy: ResolutionStrategy, module?: string): void {
  if (module) {
    moduleStrategies[module] = strategy;
    log.info(`[ConflictResolution] Set strategy for module ${module}: ${strategy}`);
  } else {
    globalStrategy = strategy;
    log.info(`[ConflictResolution] Set global strategy: ${strategy}`);
  }
}

export function getResolutionStrategy(module?: string): ResolutionStrategy {
