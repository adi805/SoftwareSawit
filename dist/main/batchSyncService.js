...
async function processSingleBatch(apiClient, items, config, batchNumber) {
    ...
    retryService.setOnRetryExhausted((itemId, finalError) => {
        electron_log_1.default.warn(`[BatchSync] All retries exhausted for item ${itemId}: ${finalError}`);
        ...
        for (const itemResult of result.results) {
            if (itemResult.success) {
                ...
                if (classification.canRetry && originalItem) {
                    ...
                    const retryDecision = retryService.processFailedSync(itemResult.itemId, originalItem.module, originalItem.operation, originalItem.recordId, errorMessage, itemResult.statusCode, async () => {
                    ...
                    if (retryDecision.shouldRetry) {
                        electron_log_1.default.info(`[BatchSync] Scheduled retry for ${itemResult.itemId}: ${retryDecision.message}`);
...
async function processSingleItem(apiClient, item, config) {
    ...
    const retryResult = retryService.processFailedSync(item.id, item.module, item.operation, item.recordId, 'Initial attempt', undefined, // No error yet
    async () => {
    ...
    try {
        ...
        const classification = retryService.classifyError(result.error || 'Unknown error', result.statusCode);
        ...
        const retryDecision = retryService.processFailedSync(item.id, item.module, item.operation, item.recordId, classification.errorMessage, result.statusCode, async () => {
        ...
        // Retry was scheduled
        electron_log_1.default.info(`[BatchSync] Retry scheduled for ${item.id}: ${retryDecision.message}`);
        return {
            success: false,
            error: classification.errorMessage,
            statusCode: result.statusCode,
        ...
        electron_log_1.default.info(`[BatchSync] Retrying ${item.id} after ${delayMs}ms (attempt ${currentAttempt + 1}/${config.maxRetries})`);
        await sleep(delayMs);
