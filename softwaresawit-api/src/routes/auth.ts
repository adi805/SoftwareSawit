...
export const moduleAccessMiddleware = (requiredModule: string) => {
  return async (c: any, next: () => Promise<void>) => {
    ...
    const userModules = payload.modules as string[] | undefined;
  ...
  try {
    ...
    return c.json({
      ...
      user: {
        ...
        nama: user.nama,
        role: user.role,
        modules: user.modules ? JSON.parse(user.modules as string) : ['kas'],
      }
    });
  } catch (error: any) {
