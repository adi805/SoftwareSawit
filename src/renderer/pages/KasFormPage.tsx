...
const KasFormPage: React.FC<KasFormPageProps> = ({ transaction, onSave, onCancel, currentUser }) => {
  const isEdit = !!transaction;
  const [transactionType, setTransactionType] = useState<'Kas Masuk' | 'Kas Keluar'>(
    transaction?.transaction_type || 'Kas Masuk'
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        const result = await window.electronAPI.localFirstUpdateKasTransaction(
          transaction.id,
          {
        ...
        if (result.success) {
          setSubmitSuccess('Transaksi berhasil diupdate (akan disinkronkan)');
        ...
        const result = await window.electronAPI.localFirstCreateKasTransaction({
        ...
        if (result.success) {
          setSubmitSuccess('Transaksi berhasil ditambahkan (akan disinkronkan)');
          setTimeout(() => onSave(), 1500);
        } else {
          setSubmitError(result.message);
        }
      ...
      console.error('[KAS] Submit error:', error);
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold text-gray-800">
              {isEdit ? 'Edit Transaksi Kas' : 'Tambah Transaksi Kas'}
            </h1>
            <p className="text-sm text-gray-500 mt-1">
              {isEdit 
                ? `Edit transaksi ${transaction?.transaction_number}` 
                : 'Tambah transaksi kas masuk atau kas keluar'}
            </p>
          </div>
          <button
            ...
            {!isEdit && (
              <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <button
                    type="button"
                    onClick={() => setTransactionType('Kas Masuk')}
                  ...
                  >
                    ...
                    <p className="text-xs mt-1 text-gray-500">Penerimaan kas</p>
                  </button>
                  <button
                    type="button"
                    onClick={() => setTransactionType('Kas Keluar')}
                    className={`px-4 py-3 rounded-lg border-2 text-sm font-medium transition-colors ${
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8v8m0-8l-4 4m4-4l4 4M5 8h14v8H5z" />
                      </svg>
                      Kas Keluar
                    </div>
                    <p className="text-xs mt-1 text-gray-500">Pengeluaran kas</p>
                  </button>
                </div>
              </div>
            )}
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <ul className="text-xs text-blue-600 mt-1 space-y-1">
                    ...
                    <li>• Transaksi belum di-approve tidak mempengaruhi saldo kas</li>
                  </ul>
                </div>
              </div>
            </div>
