      ...
      for (const op of allData) {
        try {
          ...
          const remainingIndices = allData.filter((_, i) => i > op.index).map(op2 => ({
            index: (operations.indexOf(op2 as any)),
            operation: (op2 as any).operation,
            recordId: (op2 as any).recordId,
            success: false,
...
async function processOperation(
  ...
  module: string,
  periode: string,
  op: { index: number; operation: string; recordId?: string; data?: Record<string, any>; timestamp?: string; deviceId?: string }
) {
  ...
  switch (op.operation) {
    case 'create': {
      ...
      const fields: string[] = ['id', 'sync_status', 'modified_at', 'created_at', 'updated_at'];
      const values: any[] = [id, 'synced', now, now, now];
