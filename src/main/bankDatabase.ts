...
// Transaction revision interface (Bank-specific with bank_account)
export interface BankTransactionRevision {
  id: string;
  transaction_id: string;
  module: 'bank';
...
function ensureDataDirectory(): void {
  const dataDir = path.join(app.getPath('userData'), 'data', 'bank', String(currentYear));
  if (!fs.existsSync(dataDir)) {
    fs.mkdirSync(dataDir, { recursive: true });
    log.info(`[BANK-DB] Created data directory: ${dataDir}`);
  }
}

// Initialize database
export async function initBankDatabase(): Promise<void> {
  log.info('[BANK-DB] Initializing Bank database...');
  ...
  try {
    ...
    if (fs.existsSync(DB_PATH)) {
      ...
      log.info('[BANK-DB] Loaded existing database:', DB_PATH);
    } else {
      db = new SQL.Database();
      log.info('[BANK-DB] Created new database:', DB_PATH);
    }
    ...
    log.info('[BANK-DB] Bank database initialized successfully');
  } catch (error) {
    log.error('[BANK-DB] Failed to initialize database:', error);
    throw error;
  }
}
...
function createTables(): void {
  ...
  log.info('[BANK-DB] Tables created');
}
...
export function switchPeriod(year: number, month: number): { success: boolean; message: string } {
  ...
  try {
    ...
    if (fs.existsSync(DB_PATH)) {
      const buffer = fs.readFileSync(DB_PATH);
      db = new SQL.Database(buffer);
      log.info('[BANK-DB] Switched to existing database:', DB_PATH);
    } else {
      db = new SQL.Database();
      createTables();
      saveDatabase();
      log.info('[BANK-DB] Switched to new database:', DB_PATH);
    }

    return { success: true, message: `Switched to ${year}/${String(month).padStart(2, '0')}` };
  } catch (error) {
    log.error('[BANK-DB] Failed to switch period:', error);
...
function generateTransactionNumber(type: TransactionType): string {
  const prefix = type === TRANSACTION_TYPES.BANK_MASUK ? 'BANK-M' : 'BANK-K';
  const now = new Date();
...
export function getAllTransactions(): BankTransactionWithDetails[] {
  if (!db) return [];
  ...
  return result[0].values.map((row) => {
    ...
    return obj as unknown as BankTransactionWithDetails;
  });
}

export function getTransactionById(id: string): BankTransactionWithDetails | null {
  if (!db) return null;
...
export function createTransaction(input: CreateTransactionInput): { success: boolean; message: string; transaction?: BankTransaction } {
  if (!db) {
    return { success: false, message: 'Database not initialized' };
  }
  ...
  log.info(`[BANK-DB] Created transaction: ${transactionNumber}`);
...
export function updateTransaction(
  ...
  log.info(`[BANK-DB] Updated transaction ID: ${id}`);
...
export function deleteTransaction(id: string): { success: boolean; message: string } {
  ...
  log.info(`[BANK-DB] Deleted transaction ID: ${id}`);

  return { success: true, message: 'Transaksi berhasil dihapus' };
}

export function clearAllBank(): { success: boolean; message: string; deletedCount: number } {
  if (!db) {
  ...
  log.info(`[BANK-DB] Cleared all Bank data. Deleted ${count} transactions.`);
...
export function approveTransaction(
  ...
  log.info(`[BANK-DB] Approved transaction ID: ${id}, new status: ${transaction.status === TRANSACTION_STATUS.PENDING_APPROVAL_1 ? TRANSACTION_STATUS.PENDING_APPROVAL_2 : TRANSACTION_STATUS.FULLY_APPROVED}`);
...
export function rejectTransaction(
  ...
  log.info(`[BANK-DB] Rejected transaction ID: ${id}`);
...
export function getApprovers(): Array<{ id: string; full_name: string }> {
  ...
  try {
    ...
    log.error('[BANK-DB] Failed to get approvers:', error);
    return [];
  }
}
...
export function getBankBalance(): { bankMasuk: number; bankKeluar: number; balance: number } {
  if (!db) {
  ...
  `);
  const bankMasuk = masukResult.length > 0 ? (masukResult[0].values[0][0] as number) : 0;
  ...
  `);
  const bankKeluar = keluarResult.length > 0 ? (keluarResult[0].values[0][0] as number) : 0;
...
export function getTransactionsWithPagination(
...
): { data: BankTransactionWithDetails[]; total: number; page: number; pageSize: number; totalPages: number } {
  if (!db) {
  ...
  const data: BankTransactionWithDetails[] = [];
  if (result.length > 0) {
    for (const row of result[0].values) {
      ...
      data.push(obj as unknown as BankTransactionWithDetails);
    }
  }
...
export function copyTransaction(id: string, createdBy: string): { success: boolean; message: string; transaction?: BankTransaction } {
  if (!db) {
    return { success: false, message: 'Database not initialized' };
  }
...
export function importBankBatch(
  data: BankImportRow[],
  createdBy: string
): BankImportResult {
  if (!db) {
  ...
  log.info(`[BANK-DB] Import batch completed: ${imported.length} imported, ${errors.length} errors`);
...
export function createRevisionRequest(
...
): { success: boolean; message: string; revision?: BankTransactionRevision } {
  if (!db) {
    return { success: false, message: 'Database not initialized' };
  }
  ...
  const revision: BankTransactionRevision = {
  ...
  log.info(`[BANK-DB] Created revision request ID: ${id} for transaction: ${transaction.transaction_number}`);

  return { success: true, message: 'Pengajuan revisi berhasil', revision };
}

export function getPendingRevisions(
  module: 'kas' | 'bank' | 'gudang' = 'bank',
  transactionId?: string
): BankTransactionRevision[] {
  if (!db) return [];
  ...
  return result[0].values.map((row) => {
    ...
    return obj as unknown as BankTransactionRevision;
  });
}

export function getRevisionById(revisionId: string): BankTransactionRevision | null {
  if (!db) return null;
...
export function getRevisionsForTransaction(transactionId: string): BankTransactionRevision[] {
  if (!db) return [];
...
export function approveRevision(
  ...
  if (revision.status === REVISION_STATUS.PENDING_REVISION_APPROVAL_1) {
    ...
    if (revision.proposed_bank_account !== null) { updates.push('bank_account = ?'); params.push(revision.proposed_bank_account); }
  ...
  saveDatabase();
  log.info(`[BANK-DB] Approved revision ID: ${revisionId}`);
...
export function rejectRevision(
  ...
  saveDatabase();
  log.info(`[BANK-DB] Rejected revision ID: ${revisionId}`);
...
export function cancelRevision(
  ...
  saveDatabase();
  log.info(`[BANK-DB] Cancelled revision ID: ${revisionId}`);
...
export function addSyncStatusColumns(): void {
  ...
  if (!hasSyncStatus) {
    try {
      ...
      saveDatabase();
      log.info('[BANK-DB] Added sync status columns to transactions table');
    } catch (error) {
      log.warn('[BANK-DB] Could not add sync status columns:', error);
    }
  }
  ...
  try {
    ...
    log.warn('[BANK-DB] Could not create sync_status index:', error);
  }
}
...
export function updateSyncStatus(
  ...
  try {
    ...
    saveDatabase();
    log.info(`[BANK-DB] Updated sync status for ${recordId}: ${status} (attempts: ${newAttempts})`);
    ...
    log.error(`[BANK-DB] Failed to update sync status for ${recordId}:`, errorMsg);
...
export function getSyncStatus(recordId: string): TransactionSyncStatusRecord | null {
  ...
  try {
    ...
    log.error(`[BANK-DB] Failed to get sync status for ${recordId}:`, error);
    return null;
  }
}
...
export function getAllWithSyncStatus(): TransactionSyncStatusRecord[] {
  ...
  try {
    ...
    log.error('[BANK-DB] Failed to get all with sync status:', error);
    return [];
  }
}
 ...
 */
export function getBySyncStatus(status: TransactionSyncStatus): BankTransactionWithDetails[] {
  if (!db) return [];
...
export function closeDatabase(): void {
  if (db) {
    ...
    db.close();
    db = null;
    log.info('[BANK-DB] Database closed');
  }
}
