 ...
 */
export async function runMigrations(db: D1Database): Promise<{ applied: string[]; errors: string[] }> {
  ...
  const appliedSet = new Set(appliedMigrations.results.map((r: any) => r.name));
 ...
 */
export async function getMigrationStatus(db: D1Database): Promise<{ applied: string[]; pending: string[] }> {
  try {
    const result = await db
      .prepare(`SELECT name FROM ${MIGRATIONS_TABLE}`)
      .all();
    
    const appliedSet = new Set(result.results.map((r: any) => r.name));
 ...
 */
export function parsePeriodeToTableSuffix(yearMonth: string): string {
 ...
 */
export function getTableName(module: 'kas' | 'bank' | 'gudang', periode: string): string {
 ...
 */
export async function ensureTransactionTableExists(
  db: D1Database,
 ...
 */
export async function getTransactionTables(
  db: D1Database,
  module: 'kas' | 'bank' | 'gudang'
): Promise<string[]> {
  const result = await db
 ...
 */
export async function transactionTableExists(
  db: D1Database,
  module: 'kas' | 'bank' | 'gudang',
  periode: string
): Promise<boolean> {
 ...
 */
export async function listAllTables(db: D1Database): Promise<string[]> {
  const result = await db
