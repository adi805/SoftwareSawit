...
exports.TRANSACTION_TYPES = exports.REVISION_STATUS = exports.TRANSACTION_STATUS = void 0;
exports.initBankDatabase = initBankDatabase;
...
exports.clearAllBank = clearAllBank;
...
exports.importBankBatch = importBankBatch;
...
exports.getRevisionsForTransaction = getRevisionsForTransaction;
exports.approveRevision = approveRevision;
exports.rejectRevision = rejectRevision;
exports.cancelRevision = cancelRevision;
exports.getRevisionStatusOptions = getRevisionStatusOptions;
exports.addSyncStatusColumns = addSyncStatusColumns;
exports.updateSyncStatus = updateSyncStatus;
exports.getSyncStatus = getSyncStatus;
exports.getAllWithSyncStatus = getAllWithSyncStatus;
exports.resetSyncStatus = resetSyncStatus;
exports.getBySyncStatus = getBySyncStatus;
...
exports.TRANSACTION_TYPES = {
    BANK_MASUK: 'Bank Masuk',
    BANK_KELUAR: 'Bank Keluar',
};
let currentYear;
let currentMonth;
let DB_PATH;
...
function ensureDataDirectory() {
    const dataDir = path.join(electron_1.app.getPath('userData'), 'data', 'bank', String(currentYear));
    if (!fs.existsSync(dataDir)) {
        fs.mkdirSync(dataDir, { recursive: true });
        electron_log_1.default.info(`[BANK-DB] Created data directory: ${dataDir}`);
    }
}
// Initialize database
async function initBankDatabase() {
    electron_log_1.default.info('[BANK-DB] Initializing Bank database...');
    try {
        ...
        if (fs.existsSync(DB_PATH)) {
            ...
            electron_log_1.default.info('[BANK-DB] Loaded existing database:', DB_PATH);
        }
        else {
            db = new SQL.Database();
            electron_log_1.default.info('[BANK-DB] Created new database:', DB_PATH);
        }
        // Create tables
        createTables();
        ...
        // Save database
        saveDatabase();
        electron_log_1.default.info('[BANK-DB] Bank database initialized successfully');
    }
    catch (error) {
        electron_log_1.default.error('[BANK-DB] Failed to initialize database:', error);
        throw error;
    }
}
function createTables() {
  ...
  `);
    ...
    electron_log_1.default.info('[BANK-DB] Tables created');
}
...
function switchPeriod(year, month) {
    ...
    try {
        ...
        if (fs.existsSync(DB_PATH)) {
            const buffer = fs.readFileSync(DB_PATH);
            db = new SQL.Database(buffer);
            electron_log_1.default.info('[BANK-DB] Switched to existing database:', DB_PATH);
        }
        else {
            db = new SQL.Database();
            createTables();
            saveDatabase();
            electron_log_1.default.info('[BANK-DB] Switched to new database:', DB_PATH);
        }
        return { success: true, message: `Switched to ${year}/${String(month).padStart(2, '0')}` };
    }
    catch (error) {
        electron_log_1.default.error('[BANK-DB] Failed to switch period:', error);
...
function generateTransactionNumber(type) {
    const prefix = type === exports.TRANSACTION_TYPES.BANK_MASUK ? 'BANK-M' : 'BANK-K';
    const now = new Date();
...
function createTransaction(input) {
    ...
    db.run(`INSERT INTO transactions (
      ...
      coa_id, aspek_kerja_id, blok_id, bank_account, status, created_by, created_at, updated_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
        id,
        transactionNumber,
    ...
    electron_log_1.default.info(`[BANK-DB] Created transaction: ${transactionNumber}`);
...
function updateTransaction(id, input, updatedBy) {
    ...
    saveDatabase();
    electron_log_1.default.info(`[BANK-DB] Updated transaction ID: ${id}`);
...
function deleteTransaction(id) {
    ...
    saveDatabase();
    electron_log_1.default.info(`[BANK-DB] Deleted transaction ID: ${id}`);
...
function clearAllBank() {
    ...
    if (count === 0) {
        return { success: true, message: 'Tidak ada data Bank untuk dihapus', deletedCount: 0 };
    }
    db.run('DELETE FROM approval_history');
    db.run('DELETE FROM transactions');
    saveDatabase();
    electron_log_1.default.info(`[BANK-DB] Cleared all Bank data. Deleted ${count} transactions.`);
    return { success: true, message: `Berhasil menghapus ${count} data Bank`, deletedCount: count };
}
function approveTransaction(id, input) {
    ...
    saveDatabase();
    electron_log_1.default.info(`[BANK-DB] Approved transaction ID: ${id}, new status: ${transaction.status === exports.TRANSACTION_STATUS.PENDING_APPROVAL_1 ? exports.TRANSACTION_STATUS.PENDING_APPROVAL_2 : exports.TRANSACTION_STATUS.FULLY_APPROVED}`);
...
function rejectTransaction(id, input) {
    ...
    saveDatabase();
    electron_log_1.default.info(`[BANK-DB] Rejected transaction ID: ${id}`);
...
function getApprovers() {
    ...
    `);
        ...
        electron_log_1.default.error('[BANK-DB] Failed to get approvers:', error);
        return [];
    }
}
...
function getBankBalance() {
    if (!db) {
        return { bankMasuk: 0, bankKeluar: 0, balance: 0 };
    }
    // Sum fully approved Bank Masuk
    const masukResult = db.exec(`
    SELECT COALESCE(SUM(amount), 0) as total 
    FROM transactions 
    WHERE transaction_type = 'Bank Masuk' AND status = 'Fully Approved'
  `);
    const bankMasuk = masukResult.length > 0 ? masukResult[0].values[0][0] : 0;
    // Sum fully approved Bank Keluar
    const keluarResult = db.exec(`
    SELECT COALESCE(SUM(amount), 0) as total 
    FROM transactions 
    WHERE transaction_type = 'Bank Keluar' AND status = 'Fully Approved'
  `);
    const bankKeluar = keluarResult.length > 0 ? keluarResult[0].values[0][0] : 0;
    return {
        bankMasuk,
        bankKeluar,
        balance: bankMasuk - bankKeluar,
    };
}
// Search and Filter
...
function getTransactionsWithPagination(page = 1, pageSize = 20, searchTerm, transactionType, status, startDate, endDate, syncStatusFilter) {
    if (!db) {
...
function importBankBatch(data, createdBy) {
    if (!db) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        if (!row.transaction_type || row.transaction_type.trim() === '') {
            errors.push({ row: rowNum, field: 'transaction_type', message: 'Jenis transaksi harus diisi (Bank Masuk/Bank Keluar)', value: row.transaction_type || '' });
            continue;
        }
        ...
        if (transactionType !== 'Bank Masuk' && transactionType !== 'Bank Keluar') {
            errors.push({ row: rowNum, field: 'transaction_type', message: 'Jenis transaksi harus "Bank Masuk" atau "Bank Keluar"', value: row.transaction_type });
            continue;
        }
        // Validate amount
        ...
        const tempPrefix = transactionType === 'Bank Masuk' ? 'BANK-M' : 'BANK-K';
    ...
    electron_log_1.default.info(`[BANK-DB] Import batch completed: ${imported.length} imported, ${errors.length} errors`);
    return {
 ...
 */
function createRevisionRequest(transactionId, requestedBy, input) {
    if (!db) {
        return { success: false, message: 'Database not initialized' };
    }
    ...
    if (transaction.status !== exports.TRANSACTION_STATUS.FULLY_APPROVED) {
  ...
  `, [transactionId]);
    ...
    INSERT INTO transaction_revisions (
      ...
      original_amount, original_description, original_coa_id, original_aspek_kerja_id, original_blok_id, original_bank_account,
      proposed_transaction_date, proposed_amount, proposed_description,
      proposed_coa_id, proposed_aspek_kerja_id, proposed_blok_id, proposed_bank_account,
  ...
  `, [
        ...
        transaction.amount, transaction.description, transaction.coa_id, transaction.aspek_kerja_id, transaction.blok_id, transaction.bank_account,
        input.proposed_transaction_date ?? null, input.proposed_amount ?? null, input.proposed_description ?? null,
        input.proposed_coa_id ?? null, input.proposed_aspek_kerja_id ?? null, input.proposed_blok_id ?? null, input.proposed_bank_account ?? null,
        input.revision_reason.trim(), requestedBy, now,
        exports.REVISION_STATUS.PENDING_REVISION_APPROVAL_1,
        now, now,
    ]);
    const historyId = (0, uuid_1.v4)();
    db.run(`INSERT INTO approval_history (id, transaction_id, action, user_id, user_name, action_at, notes)
     VALUES (?, ?, ?, ?, ?, ?, ?)`, [historyId, transactionId, 'revision_requested', requestedBy, requestedBy, now, `Pengajuan revisi: ${input.revision_reason.trim()}`]);
    saveDatabase();
    const revision = {
        id, transaction_id: transactionId, module: 'bank',
        ...
        original_bank_account: transaction.bank_account,
    ...
    electron_log_1.default.info(`[BANK-DB] Created revision request ID: ${id} for transaction: ${transaction.transaction_number}`);
    return { success: true, message: 'Pengajuan revisi berhasil', revision };
}
function getPendingRevisions(module = 'bank', transactionId) {
    if (!db)
        return [];
    let query = `
...
function getRevisionsForTransaction(transactionId) {
    if (!db)
        return [];
    const result = db.exec(`
...
function approveRevision(revisionId, input) {
    if (!db)
        return { success: false, message: 'Database not initialized' };
    ...
    const now = new Date().toISOString();
    if (revision.status === exports.REVISION_STATUS.PENDING_REVISION_APPROVAL_1) {
        db.run(`
      UPDATE transaction_revisions SET 
        status = ?, revision_approver_1_id = ?, revision_approver_1_name = ?, revision_approver_1_at = ?, updated_at = ?
      WHERE id = ?
    `, [exports.REVISION_STATUS.PENDING_REVISION_APPROVAL_2, input.approver_id, input.approver_name, now, now, revisionId]);
        db.run(`INSERT INTO approval_history (id, transaction_id, action, user_id, user_name, action_at, notes) VALUES (?, ?, ?, ?, ?, ?, ?)`, [(0, uuid_1.v4)(), revision.transaction_id, 'revision_approved_1', input.approver_id, input.approver_name, now, 'Approval revisi tahap 1']);
    }
    else if (revision.status === exports.REVISION_STATUS.PENDING_REVISION_APPROVAL_2) {
      ...
      WHERE id = ?
    `, [exports.REVISION_STATUS.APPROVED, input.approver_id, input.approver_name, now, now, now, revisionId]);
        // Apply revision to transaction
        const updates = [];
        const params = [];
        if (revision.proposed_transaction_date !== null) {
            updates.push('transaction_date = ?');
            params.push(revision.proposed_transaction_date);
        }
        if (revision.proposed_amount !== null) {
            updates.push('amount = ?');
            params.push(revision.proposed_amount);
        }
        if (revision.proposed_description !== null) {
            updates.push('description = ?');
            params.push(revision.proposed_description);
        }
        if (revision.proposed_coa_id !== null) {
            updates.push('coa_id = ?');
            params.push(revision.proposed_coa_id);
        }
        if (revision.proposed_aspek_kerja_id !== null) {
            updates.push('aspek_kerja_id = ?');
            params.push(revision.proposed_aspek_kerja_id);
        }
        if (revision.proposed_blok_id !== null) {
            updates.push('blok_id = ?');
            params.push(revision.proposed_blok_id);
        }
        if (revision.proposed_bank_account !== null) {
            updates.push('bank_account = ?');
            params.push(revision.proposed_bank_account);
        }
        if (updates.length > 0) {
            updates.push('updated_at = ?');
            params.push(now);
        ...
        db.run(`INSERT INTO approval_history (id, transaction_id, action, user_id, user_name, action_at, notes) VALUES (?, ?, ?, ?, ?, ?, ?)`, [(0, uuid_1.v4)(), revision.transaction_id, 'revision_approved_2', input.approver_id, input.approver_name, now, 'Approval revisi tahap 2 - Revisi diterapkan']);
    }
    else {
        return { success: false, message: `Revisi dengan status "${revision.status}" tidak dapat disetujui` };
    }
    saveDatabase();
    electron_log_1.default.info(`[BANK-DB] Approved revision ID: ${revisionId}`);
    return { success: true, message: 'Revisi berhasil disetujui' };
}
function rejectRevision(revisionId, input) {
    if (!db)
        return { success: false, message: 'Database not initialized' };
    if (!input.reason || input.reason.trim() === '') {
        return { success: false, message: 'Alasan penolakan harus diisi' };
    ...
    if (!revision)
        return { success: false, message: 'Revisi tidak ditemukan' };
    if (revision.status !== exports.REVISION_STATUS.PENDING_REVISION_APPROVAL_1 &&
        revision.status !== exports.REVISION_STATUS.PENDING_REVISION_APPROVAL_2) {
    ...
    WHERE id = ?
  `, [exports.REVISION_STATUS.REJECTED, input.rejected_by_id, input.rejected_by_name, now, input.reason.trim(), now, revisionId]);
    db.run(`INSERT INTO approval_history (id, transaction_id, action, user_id, user_name, action_at, notes) VALUES (?, ?, ?, ?, ?, ?, ?)`, [(0, uuid_1.v4)(), revision.transaction_id, 'revision_rejected', input.rejected_by_id, input.rejected_by_name, now, `Revisi ditolak: ${input.reason.trim()}`]);
    saveDatabase();
    electron_log_1.default.info(`[BANK-DB] Rejected revision ID: ${revisionId}`);
    return { success: true, message: 'Revisi berhasil ditolak' };
}
function cancelRevision(revisionId, cancelledBy) {
    if (!db)
        return { success: false, message: 'Database not initialized' };
    const revision = getRevisionById(revisionId);
    if (!revision)
  ...
  `, [exports.REVISION_STATUS.CANCELLED, now, revisionId]);
    db.run(`INSERT INTO approval_history (id, transaction_id, action, user_id, user_name, action_at, notes) VALUES (?, ?, ?, ?, ?, ?, ?)`, [(0, uuid_1.v4)(), revision.transaction_id, 'revision_cancelled', cancelledBy, cancelledBy, now, 'Revisi dibatalkan oleh pengaju']);
    saveDatabase();
    electron_log_1.default.info(`[BANK-DB] Cancelled revision ID: ${revisionId}`);
    return { success: true, message: 'Revisi berhasil dibatalkan' };
}
function getRevisionStatusOptions() {
    return [
        exports.REVISION_STATUS.PENDING_REVISION_APPROVAL_1,
        exports.REVISION_STATUS.PENDING_REVISION_APPROVAL_2,
        exports.REVISION_STATUS.APPROVED,
        exports.REVISION_STATUS.REJECTED,
        exports.REVISION_STATUS.CANCELLED,
    ];
}
/**
...
function addSyncStatusColumns() {
    ...
    if (!hasSyncStatus) {
        try {
            ...
            saveDatabase();
            electron_log_1.default.info('[BANK-DB] Added sync status columns to transactions table');
        }
        catch (error) {
            electron_log_1.default.warn('[BANK-DB] Could not add sync status columns:', error);
        }
    }
    ...
    try {
        ...
        electron_log_1.default.warn('[BANK-DB] Could not create sync_status index:', error);
    }
}
/**
 * Update sync status for a transaction
 */
function updateSyncStatus(recordId, status, errorMessage) {
    if (!db) {
        return { success: false, message: 'Database not initialized' };
    }
    try {
        // Get current attempts count
        const currentResult = db.exec('SELECT sync_attempts FROM transactions WHERE id = ?', [recordId]);
        const currentAttempts = currentResult.length > 0 && currentResult[0].values.length > 0
            ? currentResult[0].values[0][0] || 0
            : 0;
        const now = new Date().toISOString();
        ...
        last_sync_at = ?,
        sync_error = ?
      WHERE id = ?`, [status, newAttempts, now, errorMessage || null, recordId]);
        saveDatabase();
        electron_log_1.default.info(`[BANK-DB] Updated sync status for ${recordId}: ${status} (attempts: ${newAttempts})`);
        return {
            success: true,
        ...
        electron_log_1.default.error(`[BANK-DB] Failed to update sync status for ${recordId}:`, errorMsg);
 ...
 */
function getSyncStatus(recordId) {
    if (!db)
        return null;
    try {
        const result = db.exec(`SELECT id, sync_status, sync_attempts, last_sync_at, sync_error
       FROM transactions WHERE id = ?`, [recordId]);
        if (result.length === 0 || result[0].values.length === 0) {
            return null;
        }
        ...
        return {
            ...
            module: 'bank',
            recordId: row[0],
            syncStatus: row[1] || 'pending',
            syncAttempts: row[2] || 0,
            lastSyncAt: row[3],
            syncError: row[4],
        };
    }
    catch (error) {
        electron_log_1.default.error(`[BANK-DB] Failed to get sync status for ${recordId}:`, error);
        return null;
    }
}
/**
...
function getAllWithSyncStatus() {
    ...
    try {
       ...
       FROM transactions`);
        ...
        electron_log_1.default.error('[BANK-DB] Failed to get all with sync status:', error);
        return [];
    }
}
/**
...
function closeDatabase() {
    if (db) {
        ...
        db.close();
        db = null;
        electron_log_1.default.info('[BANK-DB] Database closed');
    }
}
