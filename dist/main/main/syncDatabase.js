...
function getSyncQueueStats() {
  ...
  `);
    ...
    return {
        ...
        total: counts.pending + counts.in_progress + counts.failed + counts.completed,
        byModule,
...
function updateSyncQueueItemStatus(id, status, errorMessage) {
    if (!db)
        return;
    const now = new Date().toISOString();
    if (status === 'failed') {
...
function inspectQueue() {
  ...
  `);
    ...
    for (const status of ['pending', 'in_progress', 'failed', 'completed']) {
        const statusResult = db.exec(`
      SELECT id, batch_id, module, operation, record_id, data, timestamp, status, retry_count, error_message
      FROM sync_queue
      WHERE status = ?
