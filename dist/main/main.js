...
const dashboardApproval = __importStar(require("./dashboardApproval"));
const syncStatusTracking = __importStar(require("./syncStatusTracking"));
const syncHealthDashboard = __importStar(require("./syncHealthDashboard"));
// Configure logging
try {
...
function setupIpcHandlers() {
    ...
    electron_1.ipcMain.handle('user:exportDatabase', async (_event, targetPath) => {
        electron_log_1.default.debug(`[IPC] user:exportDatabase - ${targetPath}`);
        return userDb.exportUsersDatabase(targetPath);
    });
    // Import users database
    electron_1.ipcMain.handle('user:importDatabase', async (_event, sourcePath, conflictResolution) => {
        electron_log_1.default.debug(`[IPC] user:importDatabase - ${sourcePath}, resolution: ${conflictResolution}`);
        return userDb.importUsersDatabase(sourcePath, conflictResolution);
    });
    // Get users database path
    electron_1.ipcMain.handle('user:getUsersDbPath', () => {
        electron_log_1.default.debug('[IPC] user:getUsersDbPath');
    ...
    electron_1.ipcMain.handle('kas:getWithPagination', (_event, page, pageSize, searchTerm, transactionType, status, startDate, endDate, syncStatusFilter) => {
        electron_log_1.default.debug(`[IPC] kas:getWithPagination - page ${page}`);
        return kasDb.getTransactionsWithPagination(page, pageSize, searchTerm, transactionType, status, startDate, endDate, syncStatusFilter);
    });
    // Get status options
    ...
    // Get transactions with pagination
    electron_1.ipcMain.handle('bank:getWithPagination', (_event, page, pageSize, searchTerm, transactionType, status, startDate, endDate, syncStatusFilter) => {
        electron_log_1.default.debug(`[IPC] bank:getWithPagination - page ${page}`);
        return bankDb.getTransactionsWithPagination(page, pageSize, searchTerm, transactionType, status, startDate, endDate, syncStatusFilter);
    });
    // Get status options
    ...
    // Get transactions with pagination
    electron_1.ipcMain.handle('gudang:getWithPagination', (_event, page, pageSize, searchTerm, transactionType, status, startDate, endDate, syncStatusFilter) => {
        electron_log_1.default.debug(`[IPC] gudang:getWithPagination - page ${page}`);
        return gudangDb.getTransactionsWithPagination(page, pageSize, searchTerm, transactionType, status, startDate, endDate, syncStatusFilter);
    });
    // Get status options
    ...
    electron_1.ipcMain.handle('revision:getPendingRevisions', (_event, filters) => {
        electron_log_1.default.debug('[IPC] revision:getPendingRevisions');
        try {
            const allRevisions = [];
            ...
            electron_log_1.default.error('[IPC] revision:getPendingRevisions error:', error);
            return { success: false, message: 'Failed to get pending revisions', revisions: [] };
        }
    });
    // Get revision by ID
    electron_1.ipcMain.handle('revision:getById', (_event, revisionId) => {
        electron_log_1.default.debug(`[IPC] revision:getById - ${revisionId}`);
        try {
            // Try each module's getRevisionById
            let revision = kasDb.getRevisionById(revisionId);
            if (!revision)
                revision = bankDb.getRevisionById(revisionId);
            if (!revision)
                revision = gudangDb.getRevisionById(revisionId);
            if (!revision) {
            ...
            electron_log_1.default.error('[IPC] revision:getById error:', error);
            return { success: false, message: 'Failed to get revision' };
        }
    });
    // Get all revisions for a transaction
    electron_1.ipcMain.handle('revision:getForTransaction', (_event, module, transactionId) => {
        electron_log_1.default.debug(`[IPC] revision:getForTransaction - ${module}/${transactionId}`);
        try {
            let revisions;
            switch (module) {
                case 'kas':
            ...
            electron_log_1.default.error('[IPC] revision:getForTransaction error:', error);
            return { success: false, message: 'Failed to get revisions for transaction', revisions: [] };
        }
    });
    // Approve revision
    electron_1.ipcMain.handle('revision:approve', (_event, revisionId, input) => {
        electron_log_1.default.debug(`[IPC] revision:approve - ${revisionId}`);
        try {
            ...
            electron_log_1.default.error('[IPC] revision:approve error:', error);
            return { success: false, message: 'Failed to approve revision' };
        }
    });
    // Reject revision
    electron_1.ipcMain.handle('revision:reject', (_event, revisionId, input) => {
        electron_log_1.default.debug(`[IPC] revision:reject - ${revisionId}`);
        try {
            // First get the revision to know which module
            let revision = kasDb.getRevisionById(revisionId);
            if (!revision)
            ...
            electron_log_1.default.error('[IPC] revision:reject error:', error);
            return { success: false, message: 'Failed to reject revision' };
        }
    });
    // Cancel revision (by requester)
    electron_1.ipcMain.handle('revision:cancel', (_event, revisionId, cancelledBy) => {
        electron_log_1.default.debug(`[IPC] revision:cancel - ${revisionId}`);
        try {
            // First get the revision to know which module
            let revision = kasDb.getRevisionById(revisionId);
            if (!revision)
            ...
            electron_log_1.default.error('[IPC] revision:cancel error:', error);
            return { success: false, message: 'Failed to cancel revision' };
        }
    });
    // Get revision counts for summary
    electron_1.ipcMain.handle('revision:getCounts', () => {
        electron_log_1.default.debug('[IPC] revision:getCounts');
        try {
            ...
            electron_log_1.default.error('[IPC] revision:getCounts error:', error);
            return { success: false, message: 'Failed to get revision counts' };
        }
    });
    electron_log_1.default.info('[IPC] Revision workflow IPC handlers ready');
    ...
    electron_1.ipcMain.handle('sync:getModuleSyncStatus', () => {
        electron_log_1.default.debug('[IPC] sync:getModuleSyncStatus');
        try {
            ...
            electron_log_1.default.error('[IPC] sync:getModuleSyncStatus error:', error);
    ...
    electron_1.ipcMain.handle('sync:getPendingConflicts', (_event, module) => {
        electron_log_1.default.debug(`[IPC] sync:getPendingConflicts - module: ${module || 'all'}`);
        return syncDb.getPendingConflicts(module);
    });
    // Get a specific conflict by ID
    electron_1.ipcMain.handle('sync:getConflictById', (_event, id) => {
        electron_log_1.default.debug(`[IPC] sync:getConflictById - ${id}`);
        return syncDb.getConflictById(id);
    });
    // Resolve a conflict manually
    electron_1.ipcMain.handle('sync:resolveConflict', (_event, conflictId, resolution, resolvedData) => {
        electron_log_1.default.debug(`[IPC] sync:resolveConflict - ${conflictId} -> ${resolution}`);
        return syncDb.resolveConflict(conflictId, resolution, resolvedData);
    });
    // Discard a conflict
    electron_1.ipcMain.handle('sync:discardConflict', (_event, conflictId) => {
        electron_log_1.default.debug(`[IPC] sync:discardConflict - ${conflictId}`);
        return syncDb.discardConflict(conflictId);
    });
    // Get conflict statistics
    electron_1.ipcMain.handle('sync:getConflictStats', () => {
        electron_log_1.default.debug('[IPC] sync:getConflictStats');
        return syncDb.getConflictStats();
    });
    // Get sync history (VAL-UI-009)
    electron_1.ipcMain.handle('sync:getSyncHistory', (_event, page, pageSize, filter) => {
        electron_log_1.default.debug('[IPC] sync:getSyncHistory');
    ...
    electron_1.ipcMain.handle('sync:getTransactionSyncStatus', (_event, module, recordId) => {
        electron_log_1.default.debug(`[IPC] sync:getTransactionSyncStatus - ${module}/${recordId}`);
        try {
            ...
            electron_log_1.default.error('[IPC] sync:getTransactionSyncStatus error:', error);
            return { success: false, message: 'Failed to get transaction sync status' };
        }
    });
    // Get all transactions with sync status for a module
    electron_1.ipcMain.handle('sync:getTransactionsWithSyncStatus', (_event, module) => {
        electron_log_1.default.debug(`[IPC] sync:getTransactionsWithSyncStatus - ${module}`);
        try {
            const statuses = syncStatusTracking.getTransactionsWithSyncStatus(module);
            return { success: true, statuses };
        }
        catch (error) {
            electron_log_1.default.error('[IPC] sync:getTransactionsWithSyncStatus error:', error);
            return { success: false, message: 'Failed to get transactions with sync status' };
        }
    });
    // Update transaction sync status (internal use)
    electron_1.ipcMain.handle('sync:updateTransactionSyncStatus', (_event, module, recordId, status, errorMessage) => {
        electron_log_1.default.debug(`[IPC] sync:updateTransactionSyncStatus - ${module}/${recordId}: ${status}`);
        try {
            let result;
            switch (status) {
                case 'synced':
            ...
            electron_log_1.default.error('[IPC] sync:updateTransactionSyncStatus error:', error);
            return { success: false, message: 'Failed to update transaction sync status' };
        }
    });
    // Reset sync status for retry
    electron_1.ipcMain.handle('sync:resetTransactionSyncStatus', (_event, module, recordId) => {
        electron_log_1.default.debug(`[IPC] sync:resetTransactionSyncStatus - ${module}/${recordId}`);
        try {
            return syncStatusTracking.resetSyncStatusForRetry(module, recordId);
        }
        catch (error) {
            electron_log_1.default.error('[IPC] sync:resetTransactionSyncStatus error:', error);
    ...
    electron_1.ipcMain.handle('syncHealth:getStats', () => {
        electron_log_1.default.debug('[IPC] syncHealth:getStats');
        return syncHealthDashboard.getSyncStats();
    });
    // Get per-module sync status breakdown
    electron_1.ipcMain.handle('syncHealth:getModuleSyncStatus', () => {
        electron_log_1.default.debug('[IPC] syncHealth:getModuleSyncStatus');
        return syncHealthDashboard.getModuleSyncStatus();
    });
    // Get list of failed items with retry information
    electron_1.ipcMain.handle('syncHealth:getFailedItems', (_event, options) => {
        electron_log_1.default.debug('[IPC] syncHealth:getFailedItems');
        return syncHealthDashboard.getFailedItems(options);
    });
    // Retry a single failed item
    electron_1.ipcMain.handle('syncHealth:retryFailedItem', (_event, itemId) => {
        electron_log_1.default.debug(`[IPC] syncHealth:retryFailedItem - ${itemId}`);
        return syncHealthDashboard.retryFailedItem(itemId);
    });
    // Retry all failed items
    electron_1.ipcMain.handle('syncHealth:retryAllFailed', () => {
        electron_log_1.default.debug('[IPC] syncHealth:retryAllFailed');
        return syncHealthDashboard.retryAllFailed();
    });
    // Get retry configuration info
    electron_1.ipcMain.handle('syncHealth:getRetryConfig', () => {
        electron_log_1.default.debug('[IPC] syncHealth:getRetryConfig');
        return syncHealthDashboard.getRetryConfig();
    });
    // Get queue health status
    electron_1.ipcMain.handle('syncHealth:getQueueHealth', () => {
        electron_log_1.default.debug('[IPC] syncHealth:getQueueHealth');
        return syncHealthDashboard.getQueueHealth();
    });
    // Get comprehensive health dashboard data
    electron_1.ipcMain.handle('syncHealth:getHealthDashboardData', () => {
        electron_log_1.default.debug('[IPC] syncHealth:getHealthDashboardData');
        return syncHealthDashboard.getHealthDashboardData();
    });
    electron_log_1.default.info('[IPC] Sync Health Dashboard IPC handlers ready');
    ...
    electron_1.ipcMain.handle('localFirst:aspekKerja:create', (_event, kode, nama, coaId, jenis, statusAktif) => {
        electron_log_1.default.debug(`[IPC] localFirst:aspekKerja:create - ${kode}`);
        return localFirstOps.createAspekKerjaLocalFirst(kode, nama, coaId, jenis, statusAktif);
    });
    // Update Aspek Kerja (local-first)
    electron_1.ipcMain.handle('localFirst:aspekKerja:update', (_event, id, nama, coaId, jenis, statusAktif) => {
        electron_log_1.default.debug(`[IPC] localFirst:aspekKerja:update - ${id}`);
    ...
    electron_1.ipcMain.handle('localFirst:aspekKerja:delete', (_event, id) => {
        electron_log_1.default.debug(`[IPC] localFirst:aspekKerja:delete - ${id}`);
    ...
    electron_1.ipcMain.handle('networkStatus:getStatus', () => {
        electron_log_1.default.debug('[IPC] networkStatus:getStatus');
        return networkStatus.getStatus();
    });
    // Check network connectivity now
    electron_1.ipcMain.handle('networkStatus:checkNow', async () => {
        electron_log_1.default.debug('[IPC] networkStatus:checkNow');
        return await networkStatus.checkNow();
    });
    // Get connection quality
    electron_1.ipcMain.handle('networkStatus:getConnectionQuality', () => {
        electron_log_1.default.debug('[IPC] networkStatus:getConnectionQuality');
        return {
    ...
    electron_1.ipcMain.handle('networkStatus:checkCaptivePortal', async () => {
        electron_log_1.default.debug('[IPC] networkStatus:checkCaptivePortal');
        const isCaptive = await networkStatus.checkForCaptivePortal();
        return { isCaptivePortal: isCaptive };
    });
    // Check if online
    electron_1.ipcMain.handle('networkStatus:isOnline', () => {
        electron_log_1.default.debug('[IPC] networkStatus:isOnline');
        return { isOnline: networkStatus.isOnline() };
    });
    // Set network status configuration
    electron_1.ipcMain.handle('networkStatus:setConfig', (_event, config) => {
        electron_log_1.default.debug('[IPC] networkStatus:setConfig');
        networkStatus.setConfig(config);
        return { success: true, config: networkStatus.getStatus() };
    });
    electron_log_1.default.info('[IPC] Network status IPC handlers ready');
    ...
    electron_1.ipcMain.handle('dashboard:getPendingApprovals', async (_event, filters) => {
        electron_log_1.default.debug('[IPC] dashboard:getPendingApprovals');
        try {
            const approvals = await dashboardApproval.getPendingApprovals(filters);
            return { success: true, approvals };
        }
        catch (error) {
            electron_log_1.default.error('[IPC] dashboard:getPendingApprovals error:', error);
            return { success: false, message: 'Failed to get pending approvals' };
        }
    });
    // Get approval counts for summary cards
    electron_1.ipcMain.handle('dashboard:getApprovalCounts', async () => {
        electron_log_1.default.debug('[IPC] dashboard:getApprovalCounts');
        try {
            const counts = await dashboardApproval.getApprovalCounts();
            return { success: true, counts };
        }
        catch (error) {
            electron_log_1.default.error('[IPC] dashboard:getApprovalCounts error:', error);
            return { success: false, message: 'Failed to get approval counts' };
        }
    });
    // Approve transaction from dashboard
    electron_1.ipcMain.handle('dashboard:approve', async (_event, module, transactionId, input) => {
        electron_log_1.default.debug(`[IPC] dashboard:approve - ${module}/${transactionId}`);
        try {
            const result = await dashboardApproval.approveFromDashboard(module, transactionId, input);
            return result;
        }
        catch (error) {
            electron_log_1.default.error('[IPC] dashboard:approve error:', error);
            return { success: false, message: 'Failed to approve transaction' };
        }
    });
    // Reject transaction from dashboard
    electron_1.ipcMain.handle('dashboard:reject', async (_event, module, transactionId, input) => {
        electron_log_1.default.debug(`[IPC] dashboard:reject - ${module}/${transactionId}`);
        try {
            const result = await dashboardApproval.rejectFromDashboard(module, transactionId, input);
            return result;
        }
        catch (error) {
            electron_log_1.default.error('[IPC] dashboard:reject error:', error);
            return { success: false, message: 'Failed to reject transaction' };
        }
    });
    // Get transaction details from dashboard
    electron_1.ipcMain.handle('dashboard:getTransaction', async (_event, module, transactionId) => {
        electron_log_1.default.debug(`[IPC] dashboard:getTransaction - ${module}/${transactionId}`);
        try {
            const transaction = await dashboardApproval.getTransactionFromDashboard(module, transactionId);
            return { success: true, transaction };
        }
        catch (error) {
            electron_log_1.default.error('[IPC] dashboard:getTransaction error:', error);
            return { success: false, message: 'Failed to get transaction' };
        }
    });
    // Get approval history for a transaction
    electron_1.ipcMain.handle('dashboard:getApprovalHistory', async (_event, module, transactionId) => {
        electron_log_1.default.debug(`[IPC] dashboard:getApprovalHistory - ${module}/${transactionId}`);
        try {
            const history = await dashboardApproval.getApprovalHistoryFromDashboard(module, transactionId);
            return { success: true, history };
        }
        catch (error) {
            electron_log_1.default.error('[IPC] dashboard:getApprovalHistory error:', error);
            return { success: false, message: 'Failed to get approval history' };
        }
    });
    electron_log_1.default.info('[IPC] Dashboard approval IPC handlers ready');
}
