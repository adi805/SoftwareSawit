...
class LocalDatabaseManager {
     ...
     */
    addTransactionTableColumns(db, tableName) {
        ...
        if (!hasCoaId) {
            try {
                ...
                electron_log_1.default.info(`[DB-MGR] Added coa_id column to ${tableName}`);
            }
            catch (error) {
                electron_log_1.default.warn(`[DB-MGR] Could not add coa_id column to ${tableName}:`, error);
            }
        }
        ...
        if (!hasBlokId) {
            try {
                ...
                electron_log_1.default.info(`[DB-MGR] Added blok_id column to ${tableName}`);
            }
            catch (error) {
                electron_log_1.default.warn(`[DB-MGR] Could not add blok_id column to ${tableName}:`, error);
            }
        }
    }
    /**
