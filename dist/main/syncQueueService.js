...
function markFailed(id, errorMessage) {
    try {
        ...
        if (retryService.canRetry(id, currentAttempts)) {
            ...
            electron_log_1.default.info(`[SyncQueue] Calculated next retry for ${id} at ${nextRetryAt} (attempt ${currentAttempts + 1}, delay ${delayMs}ms)`);
        }
        else {
            electron_log_1.default.warn(`[SyncQueue] Max retries reached for ${id}, marking as error`);
