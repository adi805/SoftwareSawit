...
masterRoutes.post('/blok', zValidator('json', createBlokSchema), async (c) => {
    ...
    try {
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?)
      `)
            .bind(id, body.kode_blok, body.nama, body.tahun_tanam, body.luas, body.status, body.keterangan || '', body.pokok || 0, body.sph || 0, body.bulan_tanam || '', body.status_tanaman || '', now, now)
            .run();
        const result = await db
...
masterRoutes.put('/blok/:id', zValidator('json', blokSchema.partial()), async (c) => {
    ...
    try {
        ...
        if (body.pokok !== undefined) {
            updates.push('pokok = ?');
            values.push(body.pokok);
        }
        if (body.sph !== undefined) {
            updates.push('sph = ?');
            values.push(body.sph);
        }
        if (body.bulan_tanam !== undefined) {
            updates.push('bulan_tanam = ?');
            values.push(body.bulan_tanam);
        }
        if (body.status_tanaman !== undefined) {
