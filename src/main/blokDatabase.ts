...
export interface Blok {
  ...
  nama: string;
  tahun_tanam: number;
  luas: number; // in hectares (Ha)
  status: string; // TM, TBM, TTM, TLS (legacy field)
  keterangan: string | null;
  pokok: number | null; // jumlah pokok
  sph: number | null; // spacing
  bulan_tanam: string | null; // Bulan Tahun format (e.g., "Januari 2013")
...
export interface BlokWithUsage extends Blok {
  transaksi_count?: number;
}

const STATUS_OPTIONS = ['TM', 'TBM-0', 'TBM-1', 'TBM-2', 'TBM-3'];
const DB_PATH = path.join(app.getPath('userData'), 'data', 'master', 'blok.db');

// Initialize database
export async function initBlokDatabase(): Promise<void> {
  log.info('[BLOK-DB] Initializing Blok database...');
  ...
  try {
    ...
    if (!fs.existsSync(dataDir)) {
      ...
      log.info(`[BLOK-DB] Created data directory: ${dataDir}`);
    }
    ...
    if (fs.existsSync(DB_PATH)) {
      ...
      log.info('[BLOK-DB] Loaded existing database');
    } else {
      db = new SQL.Database();
      log.info('[BLOK-DB] Created new database');
    }
    ...
    log.info('[BLOK-DB] Blok database initialized successfully');
  } catch (error) {
    log.error('[BLOK-DB] Failed to initialize database:', error);
    throw error;
  }
}
...
function createTables(): void {
  ...
  log.info('[BLOK-DB] Tables created');
}

function createSampleData(): void {
  if (!db) return;

  log.info('[BLOK-DB] Creating sample Blok data...');
  ...
  for (const blok of sampleBlok) {
    db.run(
      `INSERT INTO blok (id, kode_blok, nama, tahun_tanam, luas, status, keterangan, pokok, sph, bulan_tanam, status_tanaman_2025, status_tanaman_2026, status_tanaman_2027, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [uuidv4(), blok.kode, blok.nama, blok.tahun, blok.luas, blok.status, blok.keterangan, blok.pokok, blok.sph, blok.bulan, blok.status2025, blok.status2026, blok.status2027, now, now]
    );
  }

  log.info(`[BLOK-DB] Created ${sampleBlok.length} sample Blok`);
}

function getBlokCount(): number {
  if (!db) return 0;
...
export function getAllBlok(): Blok[] {
  if (!db) return [];
  ...
  return result[0].values.map((row) => {
    ...
    return obj as unknown as Blok;
  });
}

export function getBlokById(id: string): Blok | null {
  if (!db) return null;
...
export function getBlokByKode(kodeBlok: string): Blok | null {
  if (!db) return null;
...
export function createBlok(
  ...
  status: string,
  keterangan: string | null,
  pokok: number | null = null,
  sph: number | null = null,
  bulanTanam: string | null = null,
  statusTanaman2025: string | null = null,
  statusTanaman2026: string | null = null,
  statusTanaman2027: string | null = null
): { success: boolean; message: string; blok?: Blok } {
  if (!db) {
  ...
  db.run(
    `INSERT INTO blok (id, kode_blok, nama, tahun_tanam, luas, status, keterangan, pokok, sph, bulan_tanam, status_tanaman_2025, status_tanaman_2026, status_tanaman_2027, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [id, kodeBlok, nama, tahunTanam, luas, status, keterangan, pokok, sph, bulanTanam, statusTanaman2025, statusTanaman2026, statusTanaman2027, now, now]
  );
  ...
  log.info(`[BLOK-DB] Created Blok: ${kodeBlok} - ${nama}`);

  return { success: true, message: 'Blok berhasil ditambahkan', blok };
}

export function updateBlok(
  id: string,
  nama: string,
  tahunTanam: number,
  luas: number,
  ...
  db.run(
    `UPDATE blok SET nama = ?, tahun_tanam = ?, luas = ?, status = ?, keterangan = ?, pokok = ?, sph = ?, bulan_tanam = ?, status_tanaman_2025 = ?, status_tanaman_2026 = ?, status_tanaman_2027 = ?, updated_at = ? WHERE id = ?`,
    [nama, tahunTanam, luas, status, keterangan, pokok, sph, bulanTanam, statusTanaman2025, statusTanaman2026, statusTanaman2027, now, id]
  );

  saveDatabase();

  log.info(`[BLOK-DB] Updated Blok ID: ${id}`);

  return { success: true, message: 'Blok berhasil diupdate' };
}

export function deleteBlok(id: string): { success: boolean; message: string } {
  if (!db) {
    return { success: false, message: 'Database not initialized' };
  }
  ...
  log.info(`[BLOK-DB] Deleted Blok ID: ${id}`);

  return { success: true, message: 'Blok berhasil dihapus' };
}

export function clearAllBlok(): { success: boolean; message: string; deletedCount: number } {
  if (!db) {
  ...
  log.info(`[BLOK-DB] Cleared all Blok data. Deleted ${count} records.`);
 ...
 */
export function checkBlokInTransactions(blokId: string): TransactionReferenceResult {
      ...
      for (const yearDir of yearDirs) {
        ...
        for (const dbFile of dbFiles) {
          ...
          try {
            ...
            if (!sqlJsModule) {
              log.warn(`[BLOK-DB] sql.js not initialized, skipping transaction check`);
              continue;
            }
            ...
            const refCheck = txDb.exec(
              `SELECT COUNT(*) FROM transactions WHERE blok_id = ?`,
              [blokId]
            );
            ...
            log.warn(`[BLOK-DB] Could not check ${module} database ${dbPath}: ${err}`);
          }
        }
      }
    } catch (err) {
      // Directory might not exist or be readable, skip it
      log.warn(`[BLOK-DB] Could not check ${module} directory: ${err}`);
    }
  }
...
export function getBlokWithPagination(
  page: number = 1,
  pageSize: number = 20,
  searchTerm?: string,
  tahunTanam?: number,
  status?: string
): { data: Blok[]; total: number; page: number; pageSize: number; totalPages: number } {
  if (!db) {
  ...
  if (result.length > 0) {
    for (const row of result[0].values) {
      ...
      data.push(obj as unknown as Blok);
    }
  }
...
export function getAllBlokKodes(): string[] {
  if (!db) return [];
...
export function importBlokBatch(
  data: Array<{ kode_blok: string; nama: string; tahun_tanam: string | number; luas: string | number; pokok?: number | null; sph?: number | null; bulan_tanam?: string | null; status_tanaman_2025?: string | null; status_tanaman_2026?: string | null; status_tanaman_2027?: string | null; keterangan?: string }>
): ImportResult {
  if (!db) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    let pokok: number | null = row.pokok ?? null;
    ...
    let sph: number | null = row.sph ?? null;
    ...
    // Validate status_tanaman fields if provided
    let statusTanaman2025: string | null = row.status_tanaman_2025 || null;
    if (statusTanaman2025 !== null && !STATUS_OPTIONS.includes(statusTanaman2025)) {
    ...
    let statusTanaman2026: string | null = row.status_tanaman_2026 || null;
    if (statusTanaman2026 !== null && !STATUS_OPTIONS.includes(statusTanaman2026)) {
    ...
    let statusTanaman2027: string | null = row.status_tanaman_2027 || null;
    if (statusTanaman2027 !== null && !STATUS_OPTIONS.includes(statusTanaman2027)) {
    ...
    db.run(
      `INSERT INTO blok (id, kode_blok, nama, tahun_tanam, luas, status, keterangan, pokok, sph, bulan_tanam, status_tanaman_2025, status_tanaman_2026, status_tanaman_2027, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [id, row.kode_blok.trim(), row.nama.trim(), tahunTanam, luas, statusTanaman2025 || 'TM', row.keterangan || null, pokok, sph, row.bulan_tanam || null, statusTanaman2025, statusTanaman2026, statusTanaman2027, now, now]
    );
  ...
  log.info(`[BLOK-DB] Import batch: ${imported.length} succeeded, ${errors.length} failed`);
...
// ============ ORG-1 Parsing Utility ============

export interface ORG1Row {
  costCentre: string;      // e.g., "21-2-00-01"
  kodeGL: string;          // e.g., "21-2-00-00"
  namaAkun: string;        // e.g., "TM - A1"
  tahunTanam: number | null; // e.g., 2009
  blokKode: string | null; // Extracted Blok code e.g., "A1", "B7"
  status: string | null;   // Extracted status e.g., "TM"
}

export interface ORG1ParseResult {
  success: boolean;
  message: string;
  data: ORG1Row[];
  totalRows: number;
...
export function parseORG1Data(
  ...
  for (let i = 0; i < rawData.length; i++) {
    ...
    let kodeGL = '';
    let namaAkun = '';
    let tahunTanam: number | null = null;
 ...
 */
export function mapORG1ToBlok(org1Data: ORG1Row[]): Array<{
  kode_blok: string;
  nama: string;
 ...
 */
export function compareORG1WithBlok(
  org1Data: ORG1Row[],
  existingBlok: Blok[]
): {
  inORG1NotInBlok: string[];
  inBlokNotInORG1: string[];
  matched: string[];
} {
  const org1BlokCodes = new Set(
    org1Data
