...
const BankFormPage: React.FC<BankFormPageProps> = ({ transaction, onSave, onCancel, currentUser }) => {
  const isEdit = !!transaction;
  const [transactionType, setTransactionType] = useState<'Bank Masuk' | 'Bank Keluar'>(
    transaction?.transaction_type || 'Bank Masuk'
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        const result = await window.electronAPI.localFirstUpdateBankTransaction(
          transaction.id,
          {
        ...
        const result = await window.electronAPI.localFirstCreateBankTransaction({
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold text-gray-800">
              {isEdit ? 'Edit Transaksi Bank' : 'Tambah Transaksi Bank'}
            </h1>
            <p className="text-sm text-gray-500 mt-1">
              {isEdit 
                ? `Edit transaksi ${transaction?.transaction_number}` 
                : 'Tambah transaksi bank masuk atau bank keluar'}
            </p>
          </div>
          <button
            ...
            {!isEdit && (
              <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <button
                    type="button"
                    onClick={() => setTransactionType('Bank Masuk')}
                    className={`px-4 py-3 rounded-lg border-2 text-sm font-medium transition-colors ${
                      transactionType === 'Bank Masuk'
                  ...
                  >
                    ...
                    <p className="text-xs mt-1 text-gray-500">Penerimaan bank</p>
                  </button>
                  <button
                    type="button"
                    onClick={() => setTransactionType('Bank Keluar')}
                    className={`px-4 py-3 rounded-lg border-2 text-sm font-medium transition-colors ${
                      transactionType === 'Bank Keluar'
                  ...
                  >
                    ...
                    <p className="text-xs mt-1 text-gray-500">Pengeluaran bank</p>
                  </button>
                </div>
              </div>
            )}
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <ul className="text-xs text-blue-600 mt-1 space-y-1">
                    ...
                    <li>• Transaksi belum di-approve tidak mempengaruhi saldo bank</li>
                  </ul>
                </div>
              </div>
            </div>
