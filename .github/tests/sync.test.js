...
class SyncTests {
  ...
  async navigateToSync() {
    const syncMenu = await this.window.locator('a:has-text("Sync"), a:has-text("Sinkron"), [data-testid="sync-menu"]').first();
    if (await syncMenu.count() > 0) {
      await syncMenu.click();
      await this.window.waitForTimeout(2000);
    } else {
      // Try settings
      const settingsMenu = await this.window.locator('a:has-text("Settings"), a:has-text("Pengaturan"), [data-testid="settings-menu"]').first();
      if (await settingsMenu.count() > 0) {
        await settingsMenu.click();
        await this.window.waitForTimeout(1000);
        
        const syncLink = await this.window.locator('a:has-text("Sync"), [data-testid="sync-settings"]').first();
  ...
  async testSyncConfiguration() {
    ...
    await this.runAssertion('VAL-ADMIN-001: Module-specific sync paths', async () => {
      const kasPath = await this.window.locator('input[name="kas_path"], [data-testid="kas-sync-path"]').first();
      const bankPath = await this.window.locator('input[name="bank_path"], [data-testid="bank-sync-path"]').first();
      const gudangPath = await this.window.locator('input[name="gudang_path"], [data-testid="gudang-sync-path"]').first();
    ...
    await this.runAssertion('VAL-SYNC-002: Path validation', async () => {
      ...
      if (await pathInput.count() > 0) {
        ...
        const saveBtn = await this.window.locator('button:has-text("Save"), button:has-text("Simpan"), [data-testid="save-sync"]').first();
  ...
  async testConnectionStatus() {
    ...
    await this.runAssertion('VAL-SYNC-010: Connection status indicator', async () => {
      const statusIndicator = await this.window.locator('.sync-status, .connection-status, [data-testid="sync-status"]').first();
    ...
    await this.runAssertion('VAL-SYNC-050: Connection notification area', async () => {
      const notificationArea = await this.window.locator('.notifications, .sync-notifications, [data-testid="sync-notifications"]').first();
    ...
    await this.runAssertion('VAL-SYNC-061: Offline indicator', async () => {
      const offlineBanner = await this.window.locator('.offline-banner, .connection-lost, [data-testid="offline-banner"]').first();
  ...
  async testSyncQueue() {
    ...
    await this.runAssertion('VAL-SYNC-021: Sync queue status', async () => {
      const queueStatus = await this.window.locator('.queue-status, .pending-count, [data-testid="queue-count"]').first();
    ...
    await this.runAssertion('VAL-SYNC-030: Manual sync button', async () => {
      const syncBtn = await this.window.locator('button:has-text("Sync Now"), button:has-text("Sync Sekarang"), [data-testid="manual-sync"]').first();
    ...
    await this.runAssertion('VAL-SYNC-040: Conflict detection area', async () => {
      const conflictArea = await this.window.locator('.conflicts, .conflict-list, [data-testid="sync-conflicts"]').first();
    ...
    await this.runAssertion('VAL-SYNC-051: Sync progress indicator', async () => {
      const progressBar = await this.window.locator('.sync-progress, .progress-bar, [data-testid="sync-progress"]').first();
  ...
  async testAdminFeatures() {
    ...
    await this.runAssertion('VAL-ADMIN-010: All location statuses', async () => {
      ...
      const locationGrid = await this.window.locator('.location-grid, .sync-locations, [data-testid="sync-locations"]').first();
    ...
    await this.runAssertion('VAL-ADMIN-020: Admin settings visible', async () => {
      const settingsPanel = await this.window.locator('.admin-settings, .sync-config, [data-testid="admin-settings"]').first();
    ...
    await this.runAssertion('VAL-ADMIN-022: View/Edit mode toggle', async () => {
      const toggleBtn = await this.window.locator('button:has-text("Edit"), button:has-text("View"), .edit-mode-toggle, [data-testid="edit-toggle"]').first();
